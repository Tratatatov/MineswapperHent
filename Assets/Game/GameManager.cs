...
public class GameManager : MonoBehaviour
{
    ...
    public void ClickCross(Tile tile)
    {
        ...
        // Проверяем и открываем только верхний, нижний, левый и правый тайлы
        if (row < Height - 1) tiles[location + Width].Open(); // Верхний
        if (row > 0) tiles[location - Width].Open(); // Нижний
        if (col < Width - 1) tiles[location + 1].Open();    // Правый
        if (col > 0) tiles[location - 1].Open();     // Левый
    }
