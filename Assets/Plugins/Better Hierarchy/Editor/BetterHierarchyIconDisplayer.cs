...
namespace Utilities.BetterHierarchy
{
    ...
    public static class BetterHierarchyIconDisplayer
    {
        ...
        private static void OnHierarchyWindowItemOnGUI(int instanceID, Rect selectionRect)
        {
            ...
            var componentsLength = components != null ? components.Length : 0;

            if (components == null || componentsLength == 0)
                return;

            var content = GetTopComponentContent(components);
            ...
            var objectStatus = SetupHierarchyObjectStatus();
            ...
            HierarchyObjectStatus SetupHierarchyObjectStatus()
            {
                ...
                var objectStatus = new HierarchyObjectStatus();

                var entireRowRect = selectionRect;
                entireRowRect.x = 0;
                entireRowRect.width = short.MaxValue;
                var expandChildrenIconRect = selectionRect;
            ...
            static GUIContent GetTopComponentContent(Component[] components)
            {
                ...
                var componentType = component ? component.GetType() : typeof(Component);
        ...
        private static Component GetTopComponent(IReadOnlyList<Component> components)
        {
            ...
            var isComponentMissing = component == null;
        ...
        private static void DrawIcon(Rect selectionRect, GUIContent objContent, GameObject obj,
            ScriptIconType iconType = ScriptIconType.BigIcon)
        {
            ...
            var originalColor = GUI.color;
            {
                ...
                {
                    var transparentIconColor = new Color(originalColor.r, originalColor.g, originalColor.b, 0.5f);
        ...
        {
            var selectedAmount = Selection.instanceIDs.Length > 1
                ? Selection.instanceIDs.Length
                : additionalSelectedInstanceIDs.Count;
            var color = UnityEditorBackgroundColor.Get(hierarchyObjectStatus, hierarchyHasFocus, selectedAmount);
        ...
        private static bool HasCustomScripts(IReadOnlyList<Component> components, out Component customComponent)
        {
            ...
            var isTooManyScriptsToCheck = componentsCount > MAX_COMPONENT_CHECKS;
            customComponent = null;

            if (isTooManyScriptsToCheck)
                return true;

            var componentsToCheckAmount = Mathf.Min(componentsCount, MAX_COMPONENT_CHECKS + FIRST_COMPONENT_INDEX);

            for (var i = FIRST_COMPONENT_INDEX; i < componentsToCheckAmount; i++)
            {
                var component = components[i];
                var isCustomScript = !IsNamespaceUnityRelated(component);
        ...
        private static bool IsNamespaceUnityRelated(Component component)
        {
            ...
            var namespaceStr = component.GetType().Namespace;
