...
namespace SingularityGroup.HotReload
{
    ...
    internal class HotReloadSettingsObject : ScriptableObject
    {
        ...
        public static HotReloadSettingsObject I
        {
            get
            {
                if (_I == null) _I = LoadSettingsOrDefault();
                return _I;
            }
        }
        ...
        private const string prefabAssetPath =
            "Packages/com.singularitygroup.hotreload/Runtime/HotReloadPrompts.prefab";
        ...
        public void EnsurePrefabSetCorrectly()
        {
            ...
            if (prefab == null)
            {
                ...
                var guids = AssetDatabase.FindAssets("HotReloadPrompts t:prefab", new[] { "Assets" });
                var paths = guids.Select(guid => AssetDatabase.GUIDToAssetPath(guid));
                var promptsPrefabPath =
                    paths.FirstOrDefault(assetpath => Path.GetFileName(assetpath) == "HotReloadPrompts.prefab");
                if (promptsPrefabPath != null) prefab = AssetDatabase.LoadAssetAtPath<GameObject>(promptsPrefabPath);
            }
        ...
        [Tooltip(
            "Should the Hot Reload runtime be included in development builds? HotReload is never included in release builds.")]
        public bool IncludeInBuild = true;

        [Header("Player Settings")] public bool AllowAndroidAppToMakeHttpRequests;

        #region hidden

        /// Reference to the Prefab, for loading it at runtime
        [HideInInspector] public GameObject PromptsPrefab;
