...
namespace SingularityGroup.HotReload
{
    ...
    {
        public static async void Forget(this Task task, CancellationToken token = new())
        {
            try
            {
                await task;
                if (task.IsFaulted) throw task.Exception ?? new Exception("unknown exception " + task);
            ...
            {
                if (!token.IsCancellationRequested) Log.Exception(ex);
            }
        }
