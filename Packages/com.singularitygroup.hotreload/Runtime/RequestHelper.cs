...
namespace SingularityGroup.HotReload
{
    ...
    internal static class RequestHelper
    {
        ...
        private static readonly HttpClient[] allClients =
            { client, clientPollPatches, clientPollAssets, clientPollStatus };
        ...
        public static PatchServerInfo ServerInfo { get; private set; } = new(defaultServerHost, null, null);
        ...
        public static void SetServerInfo(PatchServerInfo info)
        {
            ...
            if (info?.customRequestOrigin != null) SetOrigin(info.customRequestOrigin);
        }
        ...
        internal static async Task<string> GetAsync(string path)
        {
            using (var www = UnityWebRequest.Get(path))
            {
                ...
                if (string.IsNullOrEmpty(www.error)) return www.downloadHandler.text;
        ...
        internal static async void PollMethodPatches(string lastPatchId, Action<MethodPatchResponse> onResponseReceived)
        {
            ...
            var body = SerializeRequestBody(new MethodPatchRequest(lastPatchId, searchPaths, TimeSpan.FromSeconds(20),
            ...
            try
            {
                var result = await PostJson(url + "/patch", body, 30, overrideClient: clientPollPatches)
                    .ConfigureAwait(false);
                if (result.statusCode == HttpStatusCode.OK)
                {
                    var responses = JsonConvert.DeserializeObject<MethodPatchResponse[]>(result.responseText);
                    await ThreadUtility.SwitchToMainThread();
                    foreach (var response in responses) onResponseReceived(response);
                }
                ...
                {
                    Log.Info("PollMethodPatches failed with code {0} {1} {2}", (int)result.statusCode,
                        result.responseText, result.exception);
                }
            }
            finally
            {
        ...
        internal static async void PollPatchStatus(Action<PatchStatusResponse> onResponseReceived,
            PatchStatus latestStatus)
        {
            ...
            try
            {
                var result = await PostJson(url + "/patchStatus", body, 30, overrideClient: clientPollStatus)
                    .ConfigureAwait(false);
                if (result.statusCode == HttpStatusCode.OK)
                {
                ...
                {
                    Log.Info("PollPatchStatus failed with code {0} {1} {2}", (int)result.statusCode,
                        result.responseText, result.exception);
                }
            }
            finally
        ...
        internal static async void PollAssetChanges(Action<string> onResponseReceived)
        {
            ...
            try
            {
                ...
                var result = await PostJson(url + "/assetChanges", body, 30, overrideClient: clientPollAssets)
                    .ConfigureAwait(false);
                ...
                {
                    Log.Info("PollAssetChanges failed with code {0} {1} {2}", (int)result.statusCode,
                        result.responseText, result.exception);
                }
            }
            finally
        ...
        public static async Task<LoginStatusResponse> RequestLogin(string email, string password, int timeoutSeconds)
        {
            ...
            if (resp.exception == null) return JsonConvert.DeserializeObject<LoginStatusResponse>(resp.responseText);
        ...
        internal static async Task<LoginStatusResponse> RequestLogout(int timeoutSeconds = 10)
        {
            ...
            if (resp.statusCode == HttpStatusCode.OK)
                ...
                {
                    return LoginStatusResponse.FromRequestError(
                        $"Deserializing response failed with {ex.GetType().Name}: {ex.Message}");
                }

            return LoginStatusResponse.FromRequestError(resp.responseText ?? "Request timeout");
        }

        internal static async Task<ActivatePromoCodeResponse> RequestActivatePromoCode(string promoCode,
            int timeoutSeconds = 20)
        {
            ...
            try
            {
                using (var resp = await client
                           .PostAsync(CreateUrl(ServerInfo) + "/activatePromoCode", new StringContent(promoCode),
        ...
        internal static async Task RequestEditorEventWithRetry(Stat stat, EditorExtraData extraData = null)
        {
            ...
            do
            {
                var resp = await RequestEditorEvent(stat, extraData);
        ...
        internal static async Task KillServerInternal()
        {
            try
            {
                using (await client.PostAsync(CreateUrl(ServerInfo) + "/kill", new StringContent(origin))
                           .ConfigureAwait(false))
                {
                }
            }
        ...
        internal static async Task<MobileHandshakeResponse> RequestHandshake(PatchServerInfo info,
            HashSet<string> defineSymbols, string projectExclusionRegex)
        {
            await ThreadUtility.SwitchToThreadPool();
        ...
        private static async Task<HttpResponse> PostJson(string uri, string json, int timeoutSeconds,
            CancellationToken token = default, HttpClient overrideClient = null)
        {
