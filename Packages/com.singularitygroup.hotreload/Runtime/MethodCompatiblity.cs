...
namespace SingularityGroup.HotReload
{
    internal static class MethodCompatiblity
    {
        ...
        {
            if (previousMethod.Name != patchMethod.Name) return false;
            //Declaring type of patch method is different from the target method but their full name (namespace + name) is equal
            if (previousMethod.DeclaringType.FullName != patchMethod.DeclaringType.FullName) return false;
            //Check in case type parameter overloads to distinguish between: void M<T>() { } <-> void M() { }
            if (previousMethod.IsGenericMethodDefinition != patchMethod.IsGenericMethodDefinition) return false;
            ...
            if (LikelyHasExplicitThis(prevParams, patchParams, previousMethod)) patchMethodHasExplicitThis = true;
            //Special edge case: User added static keyword to method. No explicit this will be generated in that case
            if (!previousMethod.IsStatic && patchMethod.IsStatic &&
                !LikelyHasExplicitThis(prevParams, patchParams, previousMethod)) patchMethodHasExplicitThis = false;
            if (patchMethodHasExplicitThis)
            {
                ...
                if (patchParams.Length == 0) return false;
                //this parameter has to be the declaring type
                if (!ParamTypeMatches(patchParams[0].ParameterType, previousMethod.DeclaringType)) return false;
        ...
        private static bool LikelyHasExplicitThis(ParameterInfo[] prevParams, ParameterInfo[] patchParams,
            MethodBase previousMethod)
        {
            if (patchParams.Length != prevParams.Length + 1) return false;
            var patchT = patchParams[0].ParameterType;
            if (!ParamTypeMatches(patchT, previousMethod.DeclaringType)) return false;
            if (prevParams.Length >= 1 && prevParams[0].ParameterType == previousMethod.DeclaringType) return false;
