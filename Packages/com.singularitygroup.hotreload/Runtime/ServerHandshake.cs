...
namespace SingularityGroup.HotReload
{
    internal class ServerHandshake
    {
        ...
        public enum Result
        {
            ...
            [Obsolete("Not needed so far", true)] Placeholder2 = 1 << 4,
        ...
        {
            if (verifiedServer != null && serverInfo == verifiedServer) return Task.FromResult(Result.Verified);
            pendingServer = serverInfo;
            if (serverInfo != null) Prompts.SetConnectionState(ConnectionSummary.Handshaking);
                ...
                if (handshakeCheck == null || handshakeCheck.IsCompleted)
                    handshakeCheck = Task.Run(async () =>
                    {
                        try
                        {
                            ...
                            firstHandshake
                                .TrySetResult(results); // VerifyResults() can also set it, this is the default fallback
                            if (decisionIsFinal) pendingServer = null;
                        }
                        catch (Exception ex)
                        {
                            Log.Exception(ex);
        ...
        private async Task<bool> VerifyResults(Result results, PatchServerInfo server)
        {
            ...
            {
                if (!firstHandshake.Task.IsCompleted) Prompts.SetConnectionState(ConnectionSummary.Connecting);
                OnVerified(server);
                return true;
            }
            ...
            if (results.HasFlag(Result.DifferentCommit))
            {
                ...
                var yes = await Prompts.ShowQuestionDialog(new QuestionDialog.Config
                {
        ...
        private static async Task<Result> RequestHandshake(PatchServerInfo info)
        {
            ...
            if (remoteBuildTarget == null)
            {
                ...
                Log.Warning(
                    "Server did not declare its current Unity activeBuildTarget in the handshake response. Will assume it is {0}.",
                    buildInfo.activeBuildTarget);
                results |= Result.QuietWarning;
            }
            else if (remoteBuildTarget != buildInfo.activeBuildTarget)
            {
                Log.Warning(
                    "Your Unity project is running on {0}. You may need to switch it to {1} for Hot Reload to work.",
                    remoteBuildTarget, buildInfo.activeBuildTarget);
                results |= Result.QuietWarning;
            }

            if (verified) results |= Result.Verified;
            return results;
        }
    }
}
