...
namespace SingularityGroup.HotReload
{
    internal static class BurstChecker
    {
        ...
        public static bool IsBurstCompiled(MethodBase method)
        {
            ...
            if (HasAttr(method.GetCustomAttributes(), blacklistAttrName)) return false;
            if (HasAttr(method.GetCustomAttributes(), whitelistAttrName)) return true;
            //Static methods inside a [BurstCompile] type are not burst compiled by default
            if (method.DeclaringType == null || method.IsStatic) return false;
            if (HasAttr(method.DeclaringType.GetCustomAttributes(), whitelistAttrName)) return true;
