...
namespace SingularityGroup.HotReload
{
    ...
    /// <remarks>
    ///     <para>
    ///         This info is used by the HotReload Server to compile your project in the same way that the Unity Player build
    ///         was compiled.<br />
    ...
    internal class BuildInfo
    {
        ...
        public HashSet<string> DefineSymbolsAsHashSet
        {
            get
            {
                ...
                if (symbols.Length == 1 && symbols[0] == string.Empty) return new HashSet<string>();
        ...
        public PatchServerInfo BuildMachineServer
        {
            get
            {
                if (buildMachineHostName == null || buildMachinePort == 0) return null;
                return new PatchServerInfo(buildMachineHostName, buildMachinePort, commitHash, null,
                    customRequestOrigin: buildMachineRequestOrigin);
            }
        }
        ...
        {
            if (commitHash == PatchServerInfo.UnknownCommitHash) return false;
        ...
        public static bool SameCommit(string commitA, string commitB)
        {
            ...
            if (commitA.Length == commitB.Length) return commitA == commitB;

            if (commitA.Length >= 6 && commitB.Length >= 6)
            {
