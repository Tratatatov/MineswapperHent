...
namespace SingularityGroup.HotReload
{
    ...
    internal static class ThreadUtility
    {
        ...
        /// </summary>
        /// <remarks>
        ///     This field is set early in [InitializeOnLoadMethod] in the editor and [RuntimeInitializeOnLoad] in playmode / for
        ///     player builds, so your code assume it is already set.
        /// </remarks>
#if UNITY_EDITOR
        ...
        public static SynchronizationContext MainContext
        {
            get
            {
                if (_cachedMainContext != null) return _cachedMainContext;
                return EditorFallbackContext.I;
            }
            private set => _cachedMainContext = value;
        }

        private class EditorFallbackContext : SynchronizationContext
        {
            public static readonly EditorFallbackContext I = new();
        ...
        public static bool ShouldLogException(Exception ex)
        {
            ...
            while ((agg = ex as AggregateException) != null) ex = agg.InnerException;
            if (ex is ThreadAbortException) return false;
            return true;
        }

        public static void LogException(Exception ex, CancellationToken token = default)
        {
            if (ShouldLogException(ex) && !token.IsCancellationRequested) Log.Exception(ex);
        }

        public static void RunOnMainThread(Action action, CancellationToken token = default)
        {
    ...
    internal struct CancellableSwitchToMainThreadAwaitable
    {
        ...
        public struct Awaiter : ICriticalNotifyCompletion
        {
            ...
            public void UnsafeOnCompleted(Action continuation)
            {
                ...
                {
                    if (!tokenCopy.IsCancellationRequested) continuation();
                }, null);
            }
        }
    }
