...
namespace SingularityGroup.HotReload
{
    internal class SymbolResolver
    {
        ...
        {
            List<Assembly> list;
            if (assembliesByName.TryGetValue(assembly, out list)) return list;
        ...
        public MethodBase Resolve(SMethod m)
        {
            ...
            for (var i = 0; i < assmeblies.Count; i++)
                try
                {
                    result = assmeblies[i].GetLoadedModules()[0]
                        .ResolveMethod(m.metadataToken, genericTypeArgs, genericMethodArgs);
                    break;
                }
                catch (Exception ex)
                {
            ...
            if (result == null) throw new SymbolResolvingFailedException(m, lastException);
            return result;
        }

        private Type[] ResolveTypes(SType[] sTypes)
        {
            if (sTypes == null) return null;
            if (sTypes.Length == 0) return Array.Empty<Type>();
            var result = new Type[sTypes.Length];
            for (var i = 0; i < sTypes.Length; i++) result[i] = Resolve(sTypes[i]);
            return result;
        }
    }
}
