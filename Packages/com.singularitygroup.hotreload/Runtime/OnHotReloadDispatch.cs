...
namespace SingularityGroup.HotReload
{
    internal static class Dispatch
    {
        ...
        {
            if (!Attribute.IsDefined(originalMethod, typeof(InvokeOnHotReloadLocal))) return;
            var attrib =
                Attribute.GetCustomAttribute(originalMethod, typeof(InvokeOnHotReloadLocal)) as InvokeOnHotReloadLocal;
            ...
            if (patchMethodParams.Length == 0)
                ...
                Log.Warning(
                    $"[{nameof(InvokeOnHotReloadLocal)}] {patchMethod.DeclaringType?.Name} {patchMethod.Name} failed. Make sure it's a method with 0 parameters either static or defined on MonoBehaviour.");
        }
        ...
        public static void OnHotReloadLocalCustom(MethodBase origianlMethod, InvokeOnHotReloadLocal attrib)
        {
            ...
            var reloadMethod = reloadForType?.GetMethod(attrib.methodToInvoke,
                BindingFlags.Instance | BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic);

            if (reloadMethod == null)
            {
                Log.Warning(
                    $"[{nameof(InvokeOnHotReloadLocal)}] failed to find method {attrib.methodToInvoke}. Make sure it exists within the same class.");
                return;
            }
            ...
            if (reloadMethod.IsStatic)
                ...
                Log.Warning(
                    $"[{nameof(InvokeOnHotReloadLocal)}] {reloadMethod.DeclaringType?.Name} {reloadMethod.Name} failed. Make sure it's a method with 0 parameters either static or defined on MonoBehaviour.");
        }

        private static List<MethodInfo> GetOrFillMethodsCacheThreaded()
        {
            if (methodsCache != null) return methodsCache;

#if UNITY_2019_1_OR_NEWER && UNITY_EDITOR
            var methodCollection = TypeCache.GetMethodsWithAttribute(typeof(InvokeOnHotReload));
            var methods = new List<MethodInfo>();
            foreach (var m in methodCollection) methods.Add(m);
#else
        ...
        private static List<MethodInfo> GetMethodsReflection()
        {
            ...
            try
            {
                ...
                {
                    if (asm.FullName == "System" ||
                        asm.FullName.StartsWith("System.",
                            StringComparison.Ordinal)) continue; // big performance optimization
                        ...
                        foreach (var type in asm.GetTypes())
                            try
                            {
                                foreach (var m in type.GetMethods(BindingFlags.Instance | BindingFlags.Static |
                                    ...
                                    try
                                    {
                                        if (Attribute.IsDefined(m, typeof(InvokeOnHotReload))) methods.Add(m);
                                    }
        ...
        private static void InvokeStaticMethod(MethodBase m, string attrName, List<MethodPatch> patchedMethods)
        {
            ...
            catch (Exception e)
            {
                if (m.GetParameters().Length != 0)
                    Log.Warning(
                        $"[{attrName}] {m.DeclaringType?.Name} {m.Name} failed. Make sure it has 0 parameters, or 1 parameter with type List<MethodPatch>. Exception:\n{e}");
                else
        ...
        private static void InvokeInstanceMethod(MethodBase m, Object go, List<MethodPatch> patchedMethods)
        {
            ...
            catch (Exception e)
            {
                if (m.GetParameters().Length != 0)
                    Log.Warning(
                        $"[InvokeOnHotReload] {m.DeclaringType?.Name} {m.Name} failed. Make sure it has 0 parameters, or 1 parameter with type List<MethodPatch>. Exception:\n{e}");
                else
        ...
        private static void InvokeInstanceMethodStatic(MethodBase m, Object go)
        {
            ...
            catch (Exception e)
            {
                if (m.GetParameters().Length != 0)
                    Log.Warning(
                        $"[InvokeOnHotReloadLocal] {m.DeclaringType?.Name} {m.Name} failed. Make sure it has 0 parameters. Exception:\n{e}");
                else
