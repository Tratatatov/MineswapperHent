    ...
    internal static class PlayerEntrypoint
    {
        ...
        private static void InitOnAppLoad()
        {
            ...
            if (!IsPlayerWithHotReload(out onlyPrefabMissing))
            {
                if (onlyPrefabMissing)
                    Log.Warning(
                        "Hot Reload is not available in this build because one or more build settings were not supported.");
                return;
            }
        ...
        private static async Task TryAutoConnect()
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(customIp)) buildInfo.buildMachineHostName = customIp;
                var customPort = PlayerPrefs.GetString("HotReloadRuntime.CustomPort", "");
                if (!string.IsNullOrEmpty(customPort)) buildInfo.buildMachinePort = int.Parse(customPort);

                if (buildInfo.BuildMachineServer == null)
                    Prompts.ShowRetryDialog(null);
                else
                    // try reach server running on the build machine.
                    TryConnect(buildInfo.BuildMachineServer, true).Forget();
            }
            catch (Exception ex)
            {
                Log.Exception(ex);
        ...
        {
            ip = ip.Trim();
            if (buildInfo == null) throw new ArgumentException("Build info not found");
            ...
            return TryConnect(buildInfo.BuildMachineServer, false);
        }
...
#else
            return await Task.Run(() => { return BuildInfo.FromJson(File.ReadAllText(path)); });
#endif
        }
        ...
        public static bool IsPlayerWithHotReload(out bool onlyPrefabMissing)
        {
            ...
            if (!IsPlayer() || !RuntimeSupportsHotReload || !HotReloadSettingsObject.I.IncludeInBuild) return false;
