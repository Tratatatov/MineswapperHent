...
namespace SingularityGroup.HotReload.Editor
{
    internal class PackageUpdateChecker
    {
        ...
        private bool warnedVersionCheckFailed;
        ...
        public async void StartCheckingForNewVersion()
        {
            ...
            for (;;)
            {
                try
                {
                    ...
                    if (newVersionDetected != null) break;
                }
                catch (Exception ex)
                {
        ...
        {
            var currentVersion = SemVersion.Parse(PackageConst.Version, true);
        ...
        private async Task PerformVersionCheck()
        {
            ...
            if (state != null)
            {
                ...
                if (DateTime.UtcNow - state.lastVersionCheck < CheckInterval) return;
            }
            ...
            else
            {
                ...
                if (response.data > currentVersion) newVersionDetected = newVersion;
            ...
            try
            {
                using (var resp = await client.GetAsync(versionUrl).ConfigureAwait(false))
                {
                    if (resp.StatusCode != HttpStatusCode.OK)
                        return Response.FromError<SemVersion>(
                            $"Request failed with statusCode: {resp.StatusCode} {resp.ReasonPhrase}");
                    ...
                    else if (!SemVersion.TryParse(value.Value<string>(), out newVersion, true))
                        return Response.FromError<SemVersion>(
                            $"Invalid version in package.json: '{value.Value<string>()}'");
                    else
        ...
        public async Task UpdatePackageAsync(SemVersion newVersion)
        {
            ...
            if (EditorUtility.DisplayDialog($"Update To v{newVersion}",
                    $"By pressing 'Update' the Hot Reload package will be updated to v{newVersion}", "Update",
                    "Cancel"))
            {
                var resp = await GetLatestPackageVersion();
                if (resp.err == null && resp.data > newVersion) newVersion = resp.data;
        ...
        private string UpdateGitUrlInManifest(SemVersion newVersion)
        {
            ...
            if (!File.Exists(manifestJsonPath)) return "Unable to find manifest.json";
        ...
        private static string TryGetManfestDeps(JObject root, out JObject deps)
        {
            ...
            if (deps == null) return "dependencies object null in manifest.json";
            return null;
        }
        ...
        private static Response<bool> IsUsingGitRepoThreaded(string packageId)
        {
            ...
            if (!fi.Exists) return "Unable to find manifest.json";
            ...
            using (var reader = fi.OpenText())
            {
                ...
                JObject deps;
                var err = TryGetManfestDeps(root, out deps);
                if (err != null) return "no dependencies specified in manifest.json";
                JToken value;
                ...
                if (pathToPackage.StartsWith("git+", StringComparison.Ordinal)) return Response.FromResult(true);
                if (pathToPackage.StartsWith("https://", StringComparison.Ordinal)) return Response.FromResult(true);
                return Response.FromResult(false);
            }
        }
        ...
        private static class Response
        {
            ...
            {
                return new Response<T>(default, error, -1);
            }
