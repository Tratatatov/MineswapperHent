...
namespace SingularityGroup.HotReload.Editor
{
    ...
    {
        public static HotReloadEventPopup I = new();
        ...
        public void Repaint()
        {
            if (open) PopupWindow.GetWindow<PopupWindow>().Repaint();
        }

        public override void OnGUI(Rect rect)
        {
            if (Event.current.type == EventType.Layout) currentState = HotReloadRunTabState.Current;
            if (HotReloadWindowStyles.windowScreenWidth <= Constants.UpgradeLicenseNoteHideWidth
                ...
                HotReloadRunTab.RenderUpgradeLicenseNote(currentState,
                    HotReloadWindowStyles.UpgradeLicenseButtonOverlayStyle);
            using (new EditorGUILayout.HorizontalScope(EditorStyles.helpBox))
            {
                using (var scope = new EditorGUILayout.ScrollViewScope(_PopupScrollPos, GUIStyle.none,
                           GUI.skin.verticalScrollbar, GUILayout.MaxHeight(495)))
                {
            ...
            var rateAppShown = HotReloadWindow.ShouldShowRateApp();
