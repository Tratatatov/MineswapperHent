...
namespace SingularityGroup.HotReload.Editor
{
    internal static class EditorIndicationState
    {
        ...
        private static readonly Dictionary<IndicationStatus, string> IndicationIcon = new()
        {
            // grey icon:
            ...
            { IndicationStatus.Undetected, HotReloadTimelineHelper.alertIconString[AlertType.UndetectedChange] },
            {
                ...
                HotReloadTimelineHelper.alertIconString[AlertType.PartiallySupportedChange]
            },
        ...
        public static readonly Dictionary<IndicationStatus, string> IndicationText = new()
        {
        ...
        private static bool SpinnerCompletedMinDuration =>
        ...
        private static IndicationStatus GetIndicationStatus()
        {
            ...
            var newStatusIsSpinner = false;
            for (var i = 0; i < SpinnerIndications.Length; i++)
                if (SpinnerIndications[i] == status)
                    newStatusIsSpinner = true;

            var latestStatusIsSpinner = false;
            for (var i = 0; i < SpinnerIndications.Length; i++)
                if (SpinnerIndications[i] == latestStatus)
                    newStatusIsSpinner = true;

            if (status == latestStatus) return status;

            if (latestStatusIsSpinner)
            {
                if (newStatusIsSpinner) return status;

                if (SpinnerCompletedMinDuration)
                {
                    latestStatus = status;
                    return status;
                }
        ...
        private static IndicationStatus GetIndicationStatusCore()
        {
            ...
            if ((EditorCodePatcher.DownloadRequired && EditorCodePatcher.DownloadStarted) ||
                (EditorCodePatcher.RequestingDownloadAndRun && !EditorCodePatcher.Starting &&
            ...
            if (EditorCodePatcher.Compiling && !EditorCodePatcher.Stopping && !EditorCodePatcher.Starting &&
            ...
            if (EditorCodePatcher.Status?.isLicensed != true && EditorCodePatcher.Status?.isFree != true &&
                EditorCodePatcher.Status?.freeSessionFinished == true)
            ...
            if (!EditorCodePatcher.Started && !EditorCodePatcher.Running) return IndicationStatus.Stopped;
            switch (EditorCodePatcher.patchStatus)
            {
                case PatchStatus.Idle:
                    if (!EditorCodePatcher.Compiling && !EditorCodePatcher.firstPatchAttempted &&
                        !EditorCodePatcher.compileError) return IndicationStatus.Started;
                    if (EditorCodePatcher._applyingFailed) return IndicationStatus.Unsupported;
                    if (EditorCodePatcher._appliedPartially) return IndicationStatus.PartiallySupported;
                    if (EditorCodePatcher._appliedUndetected) return IndicationStatus.Undetected;
