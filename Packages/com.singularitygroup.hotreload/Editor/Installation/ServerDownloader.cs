...
namespace SingularityGroup.HotReload.Editor
{
    ...
    {
        public const string InstallDescription =
            "For Hot Reload to work, additional components specific to your operating system have to be installed";
        ...
        public async Task<bool> EnsureDownloaded(ICliController cliController, CancellationToken cancellationToken)
        {
            ...
            while (!sucess)
            {
                try
                {
                    ...
                    var result = await DownloadUtility
                        .DownloadFile(GetDownloadUrl(cliController), tmpPath, this, cancellationToken)
                ...
                catch (Exception e)
                {
                    ...
                    if (errors.Add(error))
                        Log.Warning(
                            $"Download attempt failed. If the issue persists please reach out to customer support for assistance. Exception: {error}");
                }
            ...
            if (errors?.Count > 0)
            {
                ...
                RequestHelper
                    .RequestEditorEventWithRetry(
                        new Stat(StatSource.Client, StatLevel.Error, StatFeature.Editor, StatEventType.Download), data)
                    .Forget();
        ...
        {
            if (!File.Exists(PackageConst.ConfigFileName)) return false;
            ...
            if (customExecutables == null) return false;

            string customBinaryPath;
            if (!customExecutables.TryGetValue(cliController.PlatformName, out customBinaryPath)) return false;

            if (!File.Exists(customBinaryPath))
            {
                Log.Warning(
                    $"unable to find server binary for platform '{cliController.PlatformName}' at '{customBinaryPath}'. " +
            ...
            try
            {
                ...
                var copy = true;
                if (targetFile.Exists) copy = File.GetLastWriteTimeUtc(customBinaryPath) > targetFile.LastWriteTimeUtc;
                if (copy)
                {
            ...
            {
                Log.Warning(
                    "encountered exception when copying server binary in the specified custom executable path '{0}':\n{1}",
                    customBinaryPath, ex);
                return false;
            }
        }
        ...
        private static string GetDownloadUrl(ICliController cliController)
        {
            ...
            var key =
                $"{DownloadUtility.GetPackagePrefix(version)}/server/{cliController.PlatformName}/{cliController.BinaryFileName}";
