...
namespace SingularityGroup.HotReload.Editor
{
    internal static class DownloadUtility
    {
        ...
        public static async Task<DownloadResult> DownloadFile(string url, string targetFilePath,
            IProgress<float> progress, CancellationToken cancellationToken)
        {
            ...
            using (var client = HttpClientUtils.CreateHttpClient())
            {
                ...
                return await client.DownloadAsync(url, targetFilePath, progress, cancellationToken)
        ...
        {
            if (PackageConst.IsAssetStoreBuild) return $"releases/asset-store/{version.Replace('.', '-')}";
        ...
        public static async Task<DownloadResult> DownloadAsync(this HttpClient client, string requestUri,
            string destinationFilePath, IProgress<float> progress, CancellationToken cancellationToken = default)
        {
                       ...
                       .ConfigureAwait(false))
            {
                if (response.StatusCode != HttpStatusCode.OK)
                    throw new DownloadException(
                        $"Download failed with status code {response.StatusCode} and reason {response.ReasonPhrase}");
                var contentLength = response.Content.Headers.ContentLength;
                if (!contentLength.HasValue) throw new DownloadException("Download failed: Content length unknown");

                using (var fs = new FileStream(destinationFilePath, FileMode.OpenOrCreate, FileAccess.Write,
                           FileShare.None))
                using (var download = await response.Content.ReadAsStreamAsync().ConfigureAwait(false))
                {
                    ...
                    else
                    {
                        ...
                        var relativeProgress = new Progress<long>(totalBytes =>
                            progress.Report(Math.Min(99.9f, (float)totalBytes / contentLength.Value)));
