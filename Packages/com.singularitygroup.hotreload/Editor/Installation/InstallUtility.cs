...
namespace SingularityGroup.HotReload.Editor
{
    internal static class InstallUtility
    {
        ...
        public static void HandleEditorStart(string updatedFromVersion)
        {
            ...
            if (showOnStartup == ShowOnStartupEnum.Always || (showOnStartup == ShowOnStartupEnum.OnNewVersion &&
                                                              !string.IsNullOrEmpty(updatedFromVersion)))
            ...
            if (HotReloadPrefs.LaunchOnEditorStart) EditorCodePatcher.DownloadAndRun().Forget();

            RequestHelper.RequestEditorEventWithRetry(new Stat(StatSource.Client, StatLevel.Debug, StatFeature.Editor,
                StatEventType.Start)).Forget();
        }

        public static void CheckForNewInstall()
        {
            if (File.Exists(installFlagPath)) return;
        ...
        {
            if (EditorCodePatcher.licenseType == UnityLicenseType.UnityPro) RedeemLicenseHelper.I.StartRegistration();
            // Don't open Hot Reload window inside Virtual Player folder
            // This is a heuristic since user might have the main player inside VP user-created folder, but that will be rare
            if (new DirectoryInfo(Path.GetFullPath("..")).Name != "VP") HotReloadWindow.Open();
            ...
            if (HotReloadCli.CanOpenInBackground) HotReloadPrefs.DisableConsoleWindow = true;
        }
    }
}