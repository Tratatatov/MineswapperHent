...
namespace SingularityGroup.HotReload.Editor
{
    ...
    {
        public static async Task<string> Update(string version, IProgress<float> progress,
            CancellationToken cancellationToken)
        {
            ...
            var result = await DownloadUtility.DownloadFile(url, targetFilePath, proxy, cancellationToken)
        ...
        private static void DirectoryCopy(string sourceDirPath, string destDirPath)
        {
            ...
            {
                var subDirDestPath = Path.Combine(destDirPath, subdir.Name);
