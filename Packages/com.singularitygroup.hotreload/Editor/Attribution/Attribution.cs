...
namespace SingularityGroup.HotReload.Editor
{
    internal static class Attribution
    {
        ...
        private static DateTime LastAttributionEventAt
        {
            get
            {
                if (EditorPrefs.HasKey(LastLoginKey))
                    return DateTime.ParseExact(EditorPrefs.GetString(LastLoginKey), "o", CultureInfo.InvariantCulture,
                        DateTimeStyles.AssumeUniversal);
                return DateTimeOffset.FromUnixTimeSeconds(0).UtcDateTime;
            }
            set => EditorPrefs.SetString(LastLoginKey, value.ToUniversalTime().ToString("o"));
        }
        ...
        public static void RegisterLogin(LoginStatusResponse response)
        {
            ...
            if (response?.hardwareId == null) return;
