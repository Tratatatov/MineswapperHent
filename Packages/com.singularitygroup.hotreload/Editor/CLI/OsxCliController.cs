...
namespace SingularityGroup.HotReload.Editor.Cli
{
    internal class OsxCliController : ICliController
    {
        ...
        private static readonly Lazy<SemVersion> MacOSVersion = new(() =>
        {
            ...
            var osVersion = process.StandardOutput.ReadToEnd().Trim();

            SemVersion macosVersion;
            if (SemVersion.TryParse(osVersion, out macosVersion)) return macosVersion;
            // should never happen
        ...
        private static bool UseCustomConsoleApp()
        {
            return MacOSVersion.Value.Major >= 12;
        }
            ...
            if (process.WaitForExit(1000))
            {
                if (process.ExitCode != 0)
                    Log.Warning("Failed to the run the start server command. ExitCode={0}\nFilepath: {1}",
                        process.ExitCode, executableScriptPath);
            }
            else
            {
        ...
        private static void UnzipMacOsPackage(string zipPath, string unzippedFolderPath)
        {
            ...
            if (!File.Exists(zipPath)) throw new ArgumentException($"zip file not found {zipPath}", nameof(zipPath));
            ...
            process.WaitForExit();
            if (process.ExitCode != 0) throw new Exception($"unzip failed with ExitCode {process.ExitCode}");
            ...
            if (!done) throw new Exception("Failed to find .app directory and move it to " + destDir);
            //Log.Info($"did unzip to {unzippedFolderPath}");
        }
    }
}