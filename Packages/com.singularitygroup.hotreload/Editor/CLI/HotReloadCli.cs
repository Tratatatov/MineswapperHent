...
namespace SingularityGroup.HotReload.Editor.Cli
{
    ...
    public static class HotReloadCli
    {
        ...
        {
            return StartAsync(
                HotReloadPrefs.ExposeServerToLocalNetwork,
            ...
            LoginData loginData = null)
        {
            ...
            StartArgs args;
            if (TryGetStartArgs(UnityHelper.DataPath, exposeServerToNetwork, allAssetChanges, createNoWindow, loginData,
                    port, out args)) await controller.Start(args);
        }
        ...
        private static bool TryGetStartArgs(string dataPath, bool exposeServerToNetwork, bool allAssetChanges,
            bool createNoWindow, LoginData loginData, int port, out StartArgs args)
        {
            string serverDir;
            ...
            if (config.useBuiltInProjectGeneration)
            {
                ...
                {
                    Log.Warning(
                        "Failed to start the Hot Reload Server. Cannot find solution file. Please disable \"useBuiltInProjectGeneration\" in settings to enable custom project generation.");
                    args = null;
                    return false;
                }

                Log.Info(
                    "Using default project generation. If you encounter any problem with Unity's default project generation consider disabling it to use custom project generation.");
                try
                {
            ...
            var cliArguments =
                $@"-u ""{unityProjDir}"" -s ""{slnPath}"" -t ""{cliTempDir}"" -a ""{searchAssemblies}"" -ver ""{PackageConst.Version}"" -proc ""{Process.GetCurrentProcess().Id}"" -assets ""{allAssetChanges}"" -p ""{port}""";
            if (loginData != null) cliArguments += $@" -email ""{loginData.email}"" -pass ""{loginData.password}""";
        ...
        internal static async Task PrepareBuildInfoAsync()
        {
            ...
            await Task.Run(() =>
            {
                ...
                catch (Exception e)
                {
                    ...
                    {
                        Log.Warning(
                            $"Preparing build info failed! On-device functionality might not work. Exception: {e}");
                        didLogWarning = true;
                    }
                    else
                    {
...
#pragma warning restore CS0618
            ...
            var dirs = CompilationPipeline.GetSystemAssemblyDirectories(lvl);
#else
            var t =
 typeof(UnityEditor.Editor).Assembly.GetType("UnityEditor.Scripting.ScriptCompilation.MonoLibraryHelpers");
            ...
            File.WriteAllText(PackageConst.LibraryCachePath + "/systemAssemblies.json",
                JsonConvert.SerializeObject(dirs));
        }
