...
namespace SingularityGroup.HotReload.Editor.Cli
{
    internal static class CliUtils
    {
        ...
        public static string GetHotReloadTempDir()
        {
            ...
            if (projectIdentifier != null) return Path.Combine(Path.GetTempPath(), "HotReloadTemp", projectIdentifier);
...
# else
        ...
        public static string GetExecutableTargetDir()
        {
            if (PackageConst.IsAssetStoreBuild)
                return Path.Combine(GetAppDataPath(), "asset-store",
                    $"executables_{PackageConst.ServerVersion.Replace('.', '-')}");
