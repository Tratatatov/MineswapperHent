...
public class InspectorFreezeFix
{
    ...
    {
        if (!EditorCodePatcher.config.enableInspectorFreezeFix) return;
        try
        {
            ...
            {
                var isLockedValue = inspectorType.GetProperty("isLocked")?.GetValue(inspector);
                if (isLockedValue == null) continue;
                ...
                var tracker = inspectorType.GetField("m_Tracker", BindingFlags.NonPublic | BindingFlags.Instance)
                    ?.GetValue(inspector);
                if (tracker == null) continue;
                var activeEditors = tracker.GetType().GetProperty("activeEditors");
                if (activeEditors == null) continue;
    ...
    private static void RefreshInspectors(Type inspectorType)
    {
        var allInspectorsField =
            inspectorType.GetField("m_AllInspectors", BindingFlags.NonPublic | BindingFlags.Static);

        if (allInspectorsField == null) return;
        var allInspectors = allInspectorsField.GetValue(null) as IEnumerable;
        if (allInspectors == null) return;
        ...
        foreach (var inspector in allInspectors)
        {
            ...
            var forceRebuildMethod = tracker.GetType().GetMethod("ForceRebuild",
                BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance);
            if (forceRebuildMethod == null) continue;
            forceRebuildMethod.Invoke(tracker, null);
        }
    }

    private static PropertyInfo FindFieldInHierarchy(Type type, string fieldName)
    {
