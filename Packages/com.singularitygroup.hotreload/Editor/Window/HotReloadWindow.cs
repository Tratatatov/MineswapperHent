...
namespace SingularityGroup.HotReload.Editor
{
    internal class HotReloadWindow : EditorWindow
    {
        ...
        private static readonly PackageUpdateChecker packageUpdateChecker = new();
        ...
        {
            if (timer == null) timer = new Timer(OnInterval, null, 20 * 1000, 20 * 1000);
            Current = this;
            ...
            {
                minSize = new Vector2(Constants.RecompileButtonTextHideWidth + 1, Constants.EventsListHideHeight + 70);
            ...
            titleContent = new GUIContent(" Hot Reload", GUIHelper.GetInvertibleIcon(InvertibleIcon.Logo));
        ...
        private void OnGUI()
        {
            ...
            if (Event.current.type == EventType.Layout) RunTabState = HotReloadRunTabState.Current;
        ...
        internal static void RenderLogo(int width = 243)
        {
            ...
            var backgroundRect = GUILayoutUtility.GetRect(targetWidth + padding, targetHeight + padding,
                HotReloadWindowStyles.LogoStyle);
            ...
            if (tex) GUI.DrawTexture(foregroundRect, tex, ScaleMode.ScaleToFit);
        }
        ...
        private void RenderTabs()
        {
            using (new EditorGUILayout.VerticalScope(HotReloadWindowStyles.BoxStyle))
            {
                ...
                {
                    if (collapsedTab == null) collapsedTab = selectedTab;
        ...
        private void RenderBottomBar()
        {
            ...
            if (updateAvailable) RenderUpdateButton(newVersion);

            using (new EditorGUILayout.HorizontalScope("ProjectBrowserBottomBarBg", GUILayout.ExpandWidth(true),
        ...
        private static GUIStyle renderAppBoxStyle => _renderAppBoxStyle ?? (_renderAppBoxStyle =
        ...
        public static GUILayoutOption[] NonExpandableLayout => _nonExpandable ??
                                                               (_nonExpandable = new[]
                                                               {
        ...
        internal static void RenderRateApp()
        {
            ...
            using (new EditorGUILayout.VerticalScope(renderAppBoxStyle))
            {
                    ...
                    if (GUILayout.Button("Hide", NonExpandableLayout))
                    {
                        RequestHelper
                            .RequestEditorEventWithRetry(
                                new Stat(StatSource.Client, StatLevel.Debug, StatFeature.RateApp),
                                new EditorExtraData { { "dismissed", true } }).Forget();
                ...
                using (new EditorGUILayout.HorizontalScope())
                {
                    ...
                    {
                        var openedUrl = PackageConst.IsAssetStoreBuild && EditorUtility.DisplayDialog("Rate Hot Reload",
                            "Thank you for using Hot Reload!\n\nPlease consider leaving a review on the Asset Store to support us.",
                            "Open in browser", "Cancel");
                        if (openedUrl) Application.OpenURL(Constants.UnityStoreRateAppURL);
                        HotReloadPrefs.RateAppShown = true;
                        var data = new EditorExtraData();
                        if (PackageConst.IsAssetStoreBuild) data.Add("opened_url", openedUrl);
                        data.Add("enjoy_app", true);
                        RequestHelper
                            .RequestEditorEventWithRetry(
                                new Stat(StatSource.Client, StatLevel.Debug, StatFeature.RateApp), data).Forget();
                    }
        ...
        {
            if (HotReloadPrefs.RateAppShown) return false;
            var activeDays = EditorCodePatcher.GetActiveDaysForRateApp();
            if (activeDays.Count < Constants.DaysToRateApp) return false;
            return true;
        }
                ...
                using (new GUILayout.VerticalScope())
                {
                    using (new GUILayout.HorizontalScope())
                    {
                        ...
                        if (EditorGUILayout.DropdownButton(
                                new GUIContent(
                                    Regex.Replace(_showOnStartupOption.ToString(), "([a-z])([A-Z])", "$1 $2")),
                                FocusType.Passive, GUILayout.Width(110f)))
                        {
                            var menu = new GenericMenu();
                            foreach (ShowOnStartupEnum option in Enum.GetValues(typeof(ShowOnStartupEnum)))
                                menu.AddItem(
                                    new GUIContent(Regex.Replace(option.ToString(), "([a-z])([A-Z])", "$1 $2")), false,
                                    () =>
                                    {
        ...
        internal static readonly OpenURLButton autoRefreshTroubleshootingBtn =
            new("Troubleshooting", Constants.TroubleshootingURL);

        private void RenderBottomBarCore()
        {
            var troubleshootingShown = EditorCodePatcher.Started && HotReloadWindowStyles.windowScreenWidth >= 400;
            var alertsShown = EditorCodePatcher.Started &&
                              HotReloadWindowStyles.windowScreenWidth > Constants.EventFiltersShownHideWidth;
            using (new EditorGUILayout.VerticalScope())
            {
                using (new EditorGUILayout.HorizontalScope(HotReloadWindowStyles.FooterStyle))
                {
                    if (!troubleshootingShown)
                    {
                        ...
                        if (alertsShown) GUILayout.Space(-20);
                    }
                    else
                    {
                        GUILayout.Space(21);
