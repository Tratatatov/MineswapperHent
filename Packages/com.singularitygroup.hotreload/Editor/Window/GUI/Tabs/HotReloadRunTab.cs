...
namespace SingularityGroup.HotReload.Editor
{
    ...
    {
        private static readonly string[] supportedPaths = { Path.GetFullPath("Assets"), Path.GetFullPath("Plugins") };
        public string error;
        ...
        public static ErrorData GetErrorData(string errorString)
        {
            ...
            try
            {
                ...
                do
                {
                    ...
                    if (csIndex == -1) break;
                    var fileNameStartIndex = csIndex - 1;
            ...
            // Get the error
            var error = (errorString.Contains("error CS")
                ? "Compile error, "
                : "Unsupported change detected, ") + "tap here to see more.";
            var endOfError = errorString.IndexOf(". in ", StringComparison.Ordinal);
            var specialChars = "\"'/\\";
            var characters = specialChars.ToCharArray();
            var specialChar = errorString.IndexOfAny(characters);
            try
            {
                if (errorString.Contains("error CS"))
                {
                    error = errorString.Substring(errorString.IndexOf("error CS", StringComparison.Ordinal),
                        errorString.Length - errorString.IndexOf("error CS", StringComparison.Ordinal)).Trim();
            ...
            foreach (var path in supportedPaths)
            {
                ...
                while (attempt++ < 100 && !file)
                {
                    ...
                    if (lastprojectIndex == -1) break;
                    var fullCsIndex = errorString.IndexOf(".cs", lastprojectIndex, StringComparison.Ordinal);
                    var candidateAbsolutePath =
                        errorString.Substring(lastprojectIndex, fullCsIndex - lastprojectIndex + ".cs".Length);
                    var candidateRelativePath = EditorCodePatcher.GetRelativePath(candidateAbsolutePath, path);
            ...
            try
            {
                ...
                do
                {
                    ...
                    if (int.TryParse(part.Substring(1, errorString.Contains("error CS") ? y - 1 : y),
                            out lineNumber)) break;
                } while (attempt++ < 100);
            }
            catch
            {
    ...
    internal struct HotReloadRunTabState
    {
        ...
        public static HotReloadRunTabState Current => new(
            EditorIndicationState.SpinnerActive,
            EditorIndicationState.IndicationIconPath,
            EditorCodePatcher.RequestingDownloadAndRun,
            EditorCodePatcher.Starting,
            EditorCodePatcher.Stopping,
            EditorCodePatcher.Running,
            EditorCodePatcher.StartupProgress,
            EditorIndicationState.IndicationStatusText,
            EditorCodePatcher.Status,
            EditorCodePatcher.DownloadRequired,
            EditorCodePatcher.DownloadStarted,
            EditorCodePatcher.RequestingLoginInfo,
            RedeemLicenseHelper.I.RedeemStage,
            HotReloadTimelineHelper.Suggestions.Count
        );
    }
    ...
    internal struct LicenseErrorData
    {
        ...
        public LicenseErrorData(string description, bool showManageLicenseButton = false,
            string manageLicenseButtonText = "", string loginButtonText = "", bool showSupportButton = false,
            string supportButtonText = "", bool showBuyButton = false, string buyButtonText = "",
    ...
    {
        private const string assetStoreProInfo =
        ...
        private static readonly GUILayoutOption[] timelineButtonOptions =
            { GUILayout.Height(27), GUILayout.Width(100) };
        ...
        internal static LicenseErrorData defaultLicenseErrorData =
            new(
                "We apologize, an error happened while verifying your license. Please reach out to customer support for assistance.",
                showSupportButton: true, supportButtonText: ContactSupport);
        ...
        private readonly Spinner _spinner = new(85);
        ...
        public HotReloadRunTab(HotReloadWindow window) : base(window, "Run", "forward",
            "Run and monitor the current Hot Reload session.")
        {
        }
        ...
        public bool HasPayedLicense => currentState.loginStatus != null &&
                                       (currentState.loginStatus.isIndieLicense ||
                                        currentState.loginStatus.isBusinessLicense);
        ...
        private static List<AlertType> enabledFilters
        {
            get
            {
                if (_enabledFilters == null) _enabledFilters = new List<AlertType>();

                if (HotReloadPrefs.RunTabUnsupportedChangesFilter &&
                    !_enabledFilters.Contains(AlertType.UnsupportedChange))
                ...
                if (HotReloadPrefs.RunTabPartiallyAppliedPatchesFilter &&
                    !_enabledFilters.Contains(AlertType.PartiallySupportedChange))
                ...
                if (HotReloadPrefs.RunTabUndetectedPatchesFilter &&
                    !_enabledFilters.Contains(AlertType.UndetectedChange))
        ...
        private static GUIStyle openSettingsStyle => _openSettingsStyle ?? (_openSettingsStyle =
        ...
        public static GUILayoutOption[] bigButtonHeight =>
            _bigButtonHeight ?? (_bigButtonHeight = new[] { GUILayout.Height(25) });

        internal static Dictionary<string, LicenseErrorData> LicenseErrorData => _licenseErrorData ??
            (_licenseErrorData = new Dictionary<string, LicenseErrorData>
            {
                {
                    "DeviceNotLicensedException",
                    new LicenseErrorData(
                        "Another device is using your license. Please reach out to customer support for assistance.",
                        showSupportButton: true, supportButtonText: ContactSupport)
                },
                { "DeviceBlacklistedException", new LicenseErrorData("You device has been blacklisted.") },
                {
                    "DateHeaderInvalidException",
                    new LicenseErrorData(
                        "Your license is not working because your computer's clock is incorrect. Please set the clock to the correct time to restore your license.")
                },
                {
                    "DateTimeCheatingException",
                    new LicenseErrorData(
                        "Your license is not working because your computer's clock is incorrect. Please set the clock to the correct time to restore your license.")
                },
                {
                    ...
                    new LicenseErrorData(
                        "An error has occured while activating your license. Please contact customer support for assistance.",
                        showSupportButton: true, supportButtonText: ContactSupport)
                },
                {
                    "LicenseDeletedException",
                    new LicenseErrorData(
                        "Your license has been deleted. Please contact customer support for assistance.",
                        showBuyButton: true, buyButtonText: GetLicense, showSupportButton: true,
                        supportButtonText: ContactSupport)
                },
                {
                    "LicenseDisabledException",
                    new LicenseErrorData(
                        "Your license has been disabled. Please contact customer support for assistance.",
                        showBuyButton: true, buyButtonText: GetLicense, showSupportButton: true,
                        supportButtonText: ContactSupport)
                },
                {
                    ...
                    new LicenseErrorData(
                        "Your license has expired. Please renew your license subscription using the 'Upgrade License' button below and login with your email/password to activate your license.",
                        showBuyButton: true, buyButtonText: UpgradeLicense, showManageLicenseButton: true,
                        manageLicenseButtonText: ManageLicense)
                },
                {
                    "LicenseInactiveException",
                    new LicenseErrorData(
                        "Your license is currenty inactive. Please login with your email/password to activate your license.")
                },
                {
                    "LocalLicenseException",
                    new LicenseErrorData(
                        "Your license file was damaged or corrupted. Please login with your email/password to refresh your license file.")
                },
                // Note: obsolete
                {
                    "MissingParametersException",
                    new LicenseErrorData(
                        "An account already exists for this device. Please login with your existing email/password.",
                        showBuyButton: true, buyButtonText: GetLicense)
                },
                {
                    "NetworkException",
                    new LicenseErrorData(
                        "There is an issue connecting to our servers. Please check your internet connection or contact customer support if the issue persists.",
                        showSupportButton: true, supportButtonText: ContactSupport)
                },
                {
                    "TrialLicenseExpiredException",
                    new LicenseErrorData(
                        "Your trial has expired. Activate a license with unlimited usage or continue using the Free version. View available plans on our website.",
                        showBuyButton: true, buyButtonText: UpgradeLicense)
                },
                {
                    "InvalidCredentialException",
                    new LicenseErrorData(
                        "Incorrect email/password. You can find your initial password in the sign-up email.")
                },
                ...
                {
                    "LicenseIncompatibleException",
                    new LicenseErrorData("Please upgrade your license to continue using hotreload with Unity Pro.",
                        true, ManageLicense)
                }
            });
        ...
        private Texture2D GreenTexture => EditorGUIUtility.isProSkin
            ...
            : _greenTextureLight
                ? _greenTextureLight
                : _greenTextureLight = MakeTexture(0.85f);
        ...
        internal static bool ShouldRenderConsumption(HotReloadRunTabState currentState)
        {
            return currentState.running && !currentState.starting && !currentState.stopping &&
                   currentState.loginStatus?.isLicensed != true && currentState.loginStatus?.isFree != true &&
                   !EditorCodePatcher.LoginNotRequired &&
                   !(currentState.loginStatus == null || currentState.loginStatus.isFree);
        }

        private void OnGUICore()
        {
            using (var scope = new EditorGUILayout.ScrollViewScope(_runTabScrollPos, GUI.skin.horizontalScrollbar,
                       GUI.skin.verticalScrollbar,
                       GUILayout.MaxHeight(Math.Max(HotReloadWindowStyles.windowScreenHeight, 800)),
                       GUILayout.MaxWidth(Math.Max(HotReloadWindowStyles.windowScreenWidth, 800))))
            {
            ...
            var renderStart = !EditorCodePatcher.Running && !EditorCodePatcher.Starting &&
                              !currentState.requestingDownloadAndRun && currentState.redeemStage == RedeemStage.None;
            var e = Event.current;
        ...
        {
            var alertIcon = InvertibleIcon.FoldoutClosed;
            if (HotReloadTimelineHelper.expandedEntries.Contains(alertEntry)) alertIcon = InvertibleIcon.FoldoutOpen;
        ...
        private static void RenderEntries(TimelineType timelineType)
        {
            ...
            alertEntries = timelineType == TimelineType.Suggestions
                ? HotReloadTimelineHelper.Suggestions
                : HotReloadTimelineHelper.EventsTimeline;

            var skipChildren = false;
            for (var i = 0; i < alertEntries.Count; i++)
            {
                var alertEntry = alertEntries[i];
                if (i > HotReloadTimelineHelper.maxVisibleEntries && alertEntry.entryType != EntryType.Child) break;
                if (timelineType != TimelineType.Suggestions)
                {
                    ...
                    if (alertEntry.entryType == EntryType.Child && skipChildren) continue;

                    skipChildren = false;
                }

                var entryType = alertEntry.entryType;

                var title =
                    $" {alertEntry.title}{(!string.IsNullOrEmpty(alertEntry.shortDescription) ? $": {alertEntry.shortDescription}" : "")}";
                Texture2D icon = null;
                GUIStyle style;
                ...
                using (new EditorGUILayout.HorizontalScope())
                {
                    ...
                    if (entryType == EntryType.Foldout)
                        GUI.Label(new Rect(spaceRect.x + 3, spaceRect.y, 20, 20),
                            new GUIContent(GetFoldoutIcon(alertEntry)));
                    else if (entryType == EntryType.Child)
                        GUI.Label(new Rect(spaceRect.x + 26, spaceRect.y + 2, 20, 20),
                            new GUIContent(GetFoldoutIcon(alertEntry)));
                ...
                var clickableDescription = alertEntry.title == "Unsupported change" ||
                                           alertEntry.title == "Compile error" ||
                                           alertEntry.title == "Failed applying patch to method";

                if (HotReloadTimelineHelper.expandedEntries.Contains(alertEntry) ||
                    alertEntry.alertType == AlertType.CompileError)
                    using (new EditorGUILayout.VerticalScope())
                    {
                        using (new EditorGUILayout.HorizontalScope())
                        {
                            using (new EditorGUILayout.VerticalScope(entryType == EntryType.Child
                                       ? HotReloadWindowStyles.ChildEntryBoxStyle
                                       : HotReloadWindowStyles.EntryBoxStyle))
                            {
                                ...
                                {
                                    var text = alertEntry.description;
                                    GUILayout.Label(text, HotReloadWindowStyles.LabelStyle);
                                }

                                if (alertEntry.actionData != null) alertEntry.actionData.Invoke();
                                GUILayout.Space(5f);
                            }
                        }
                    }
                ...
                {
                    var isClick = GUI.Button(new Rect(startRect.x + startRect.width - 20, startRect.y + 2, 20, 20),
                        new GUIContent(GUIHelper.GetInvertibleIcon(InvertibleIcon.Close)),
                        HotReloadWindowStyles.RemoveIconStyle);
                    if (isClick)
                    {
                ...
                if (GUI.Button(new Rect(startRect) { height = endRect.y - startRect.y + endRect.height },
                        new GUIContent(""), HotReloadWindowStyles.BarBackgroundStyle) &&
                    (entryType == EntryType.Child || entryType == EntryType.Foldout)) ToggleEntry(alertEntry);

                if (alertEntry.alertType != AlertType.Suggestion && HotReloadWindowStyles.windowScreenWidth > 400 &&
                    entryType != EntryType.Child)
                    using (new EditorGUILayout.HorizontalScope())
                    {
                        var ago = DateTime.Now - alertEntry.timestamp;
                        GUI.Label(new Rect(startRect.x + startRect.width - 60, startRect.y, 80, 20),
                            ago.TotalMinutes < 1
                                ? "now"
                                : $"{(ago.TotalHours > 1 ? $"{Math.Floor(ago.TotalHours)} h " : string.Empty)}{ago.Minutes} min",
                            HotReloadWindowStyles.TimestampStyle);
                    }
        ...
        internal static void RenderBars(HotReloadRunTabState currentState)
        {
            if (currentState.suggestionCount > 0)
            {
                ...
                using (new EditorGUILayout.HorizontalScope(HotReloadWindowStyles.Section))
                {
                    using (new EditorGUILayout.VerticalScope())
                    {
                        HotReloadPrefs.RunTabEventsSuggestionsFoldout = EditorGUILayout.Foldout(
                            HotReloadPrefs.RunTabEventsSuggestionsFoldout, "", true,
                            HotReloadWindowStyles.CustomFoldoutStyle);
                        GUILayout.Space(-23);
                        if (GUILayout.Button($"Suggestions ({currentState.suggestionCount.ToString()})",
                                HotReloadWindowStyles.ClickableLabelBoldStyle, GUILayout.Height(27)))
                            HotReloadPrefs.RunTabEventsSuggestionsFoldout =
                                !HotReloadPrefs.RunTabEventsSuggestionsFoldout;
            ...
            using (new EditorGUILayout.HorizontalScope(HotReloadWindowStyles.Section))
            {
                using (new EditorGUILayout.VerticalScope())
                {
                    HotReloadPrefs.RunTabEventsTimelineFoldout = EditorGUILayout.Foldout(
                        HotReloadPrefs.RunTabEventsTimelineFoldout, "", true, HotReloadWindowStyles.CustomFoldoutStyle);
                    GUILayout.Space(-23);
                    if (GUILayout.Button("Timeline", HotReloadWindowStyles.ClickableLabelBoldStyle,
                            timelineButtonOptions))
                    ...
                    {
                        GUILayout.Space(-10);
                        var noteShown = HotReloadTimelineHelper.GetRunTabTimelineEventCount() == 0 ||
                        ...
                        using (new EditorGUILayout.HorizontalScope())
                        {
                            if (noteShown)
                            {
                                ...
                                using (new EditorGUILayout.VerticalScope())
                                {
                                    ...
                                    else if (enabledFilters.Count < 4 &&
                                             HotReloadTimelineHelper.EventsTimeline.Count != 0)
                            ...
                            else
                            {
                                ...
                                if (HotReloadTimelineHelper.EventsTimeline.Count > 0 && GUILayout.Button("Clear"))
                                {
                                    ...
                                    if (HotReloadWindow.Current) HotReloadWindow.Current.Repaint();
                                }
        ...
        internal static void RenderConsumption(LoginStatusResponse loginStatus)
        {
            ...
            {
                EditorGUILayout.HelpBox("Something went wrong. Please check your internet connection.",
                    MessageType.Warning);
            }
            else if (loginStatus.consumptionsUnavailableReason == ConsumptionsUnavailableReason.UnrecoverableError)
            {
                EditorGUILayout.HelpBox("Something went wrong. Please contact support if the issue persists.",
                    MessageType.Error);
            }
            else if (loginStatus.freeSessionFinished)
            {
                ...
                var sessionRefreshString =
                    $"Next Session: {(sessionRefreshesAt.Hours > 0 ? $"{sessionRefreshesAt.Hours}h " : "")}{sessionRefreshesAt.Minutes}min";
                HotReloadGUIHelper.HelpBox(sessionRefreshString, MessageType.Warning, 11);
            }
            else if (loginStatus.freeSessionRunning && loginStatus.freeSessionEndTime != null)
            {
                ...
                var sessionString =
                    $"Daily Session: {(sessionEndsAt.Hours > 0 ? $"{sessionEndsAt.Hours}h " : "")}{sessionEndsAt.Minutes}min Left";
                HotReloadGUIHelper.HelpBox(sessionString, MessageType.Info, 11);
            }
            else if (loginStatus.freeSessionEndTime == null)
            {
                HotReloadGUIHelper.HelpBox("Daily Session: Make code changes to start", MessageType.Info, 11);
            }
        }

        public override void Update()
        {
            if (EditorIndicationState.SpinnerActive) _repaint = true;
            if (EditorCodePatcher.DownloadRequired) _repaint = true;
            if (EditorIndicationState.IndicationIconPath == Spinner.SpinnerIconPath) _repaint = true;
            try
            {
                // workaround: hovering over non-buttons doesn't repain by default
                if (EditorWindow.mouseOverWindow == HotReloadWindow.Current) _repaint = true;
        ...
        {
            var recompileText = HotReloadWindowStyles.windowScreenWidth > Constants.RecompileButtonTextHideWidth
                ? " Recompile"
                : "";
            var recompileButton = new GUIContent(recompileText, GUIHelper.GetInvertibleIcon(InvertibleIcon.Recompile));
            if (!GUILayout.Button(recompileButton, HotReloadWindowStyles.RecompileButton)) return;
            RecompileWithChecks();
        }
        ...
        public static void RecompileWithChecks()
        {
            var firstDialoguePass = HotReloadPrefs.RecompileDialogueShown
                                    || EditorUtility.DisplayDialog(
                                        "Hot Reload auto-applies changes",
                                        "Using the Recompile button is only necessary when Hot Reload fails to apply your changes. \n\nDo you wish to proceed?",
                                        "Recompile",
                                        "Not now");
            HotReloadPrefs.RecompileDialogueShown = true;
            if (!firstDialoguePass) return;
            var secondDialoguePass = !Application.isPlaying
                                     || EditorUtility.DisplayDialog(
                                         "Stop Play Mode and Recompile?",
                                         "Using the Recompile button will stop Play Mode.\n\nDo you wish to proceed?",
                                         "Stop and Recompile",
                                         "Cancel");
            if (!secondDialoguePass) return;
            Recompile();
        }
        ...
        private void RenderIndicationButtons()
        {
            if (currentState.requestingDownloadAndRun || currentState.starting || currentState.stopping ||
                currentState.redeemStage != RedeemStage.None) return;

            if (!currentState.running && (currentState.startupProgress?.Item1 ?? 0) == 0)
            {
                var startText = HotReloadWindowStyles.windowScreenWidth > Constants.StartButtonTextHideWidth
                    ? " Start"
                    : "";
                if (GUILayout.Button(new GUIContent(startText, GUIHelper.GetInvertibleIcon(InvertibleIcon.Start)),
                        HotReloadWindowStyles.StartButton)) EditorCodePatcher.DownloadAndRun().Forget();
            }
            else if (currentState.running && !currentState.starting)
            {
                ...
                var stopText = HotReloadWindowStyles.windowScreenWidth > Constants.StartButtonTextHideWidth
                    ? " Stop"
                    : "";
                if (GUILayout.Button(new GUIContent(stopText, GUIHelper.GetInvertibleIcon(InvertibleIcon.Stop)),
                        HotReloadWindowStyles.StopButton))
        ...
        private void RenderIndicationPanel()
        {
            ...
            if (currentState.requestingDownloadAndRun || currentState.starting) RenderProgressBar();
        ...
        internal static void RenderLicenseInfo(HotReloadRunTabState currentState)
        {
            ...
            if (!showConsumptions && !showRedeem) return;
            using (new EditorGUILayout.VerticalScope())
            {
                // space needed only for consumptions because of Stop/Start button's margin
                if (showConsumptions) GUILayout.Space(6);
        ...
        private void RenderIndication()
        {
            using (new EditorGUILayout.HorizontalScope(HotReloadWindowStyles.IndicationBox))
            {
                ...
                {
                    var text = HotReloadWindowStyles.windowScreenWidth > Constants.IndicationTextHideWidth
                        ? $"  {currentState.indicationStatusText}"
                        : "";
                    if (currentState.indicationIconPath == Spinner.SpinnerIconPath)
                    {
                        GUILayout.Label(new GUIContent(text, _spinner.GetIcon()), HotReloadWindowStyles.IndicationIcon);
                    }
                    else if (currentState.indicationIconPath != null)
                    {
                        ...
                        GUILayout.Label(new GUIContent(text, GUIHelper.GetLocalIcon(currentState.indicationIconPath)),
                            style);
                    }
                }
            }
        ...
        internal static void RenderLicenseStatusInfo(HotReloadRunTabState currentState, LoginStatusResponse loginStatus,
            bool allowHide = true, bool verbose = false)
        {
            string message = null;
                    ...
                    customGUI = () =>
                    {
                        if (loginStatus.licenseExpiresAt.Date != DateTime.MaxValue.Date)
                        {
                            EditorGUILayout.LabelField(
                                $"License will renew on {loginStatus.licenseExpiresAt.ToShortDateString()}.");
                            EditorGUILayout.Space();
                        }
            ...
            if (messageType != MessageType.Info && HotReloadPrefs.ErrorHidden && allowHide) return;
            if (message != null)
            {
        ...
        internal static void RenderBusinessLicenseInfo(GUIStyle style)
        {
            ...
            using (new EditorGUILayout.HorizontalScope())
            {
                ...
                if (Event.current.type == EventType.Repaint) style.fixedHeight = GUILayoutUtility.GetLastRect().height;
                if (GUILayout.Button("Upgrade", style)) Application.OpenURL(Constants.ProductPurchaseBusinessURL);
            }
        }
        ...
        internal static void RenderIndieLicenseInfo(GUIStyle style)
        {
            ...
            if (EditorCodePatcher.licenseType == UnityLicenseType.UnityPersonalPlus)
                ...
                message =
                    "Unity Pro/Enterprise users from company with your number of employees require an Indie license. Please upgrade your license on our website.";
            else
                return;
            GUILayout.Space(8);
            using (new EditorGUILayout.HorizontalScope())
            {
                ...
                if (Event.current.type == EventType.Repaint) style.fixedHeight = GUILayoutUtility.GetLastRect().height;
                if (GUILayout.Button("Upgrade", style)) Application.OpenURL(Constants.ProductPurchaseURL);
            }
        }

        internal static string GetMessageFromError(HotReloadRunTabState currentState, string error)
        {
            if (PackageConst.IsAssetStoreBuild && error == "TrialLicenseExpiredException") return assetStoreProInfo;
        ...
        {
            if (currentState.loginStatus?.isFree == true) return default;
            if (currentState.loginStatus == null || (string.IsNullOrEmpty(error) &&
                                                     (!currentState.loginStatus.isLicensed ||
                                                      currentState.loginStatus.isTrial)))
                return new LicenseErrorData(null, showBuyButton: true, buyButtonText: GetLicense);
            if (string.IsNullOrEmpty(error)) return default;
            if (!LicenseErrorData.ContainsKey(error)) return defaultLicenseErrorData;
        ...
        private static void RenderLicenseActionButtons(HotReloadRunTabState currentState)
        {
            ...
            if (errInfo.showBuyButton || errInfo.showManageLicenseButton)
                using (new EditorGUILayout.HorizontalScope())
                {
                    if (errInfo.showBuyButton) RenderBuyLicenseButton(errInfo.buyButtonText);
        ...
        internal static void RenderLicenseInfo(HotReloadRunTabState currentState, LoginStatusResponse loginStatus,
            bool verbose = false, bool allowHide = true, string overrideActionButton = null,
            bool showConsumptions = false)
        {
            HotReloadPrefs.ShowLogin = EditorGUILayout.Foldout(HotReloadPrefs.ShowLogin, "Hot Reload License", true,
                HotReloadWindowStyles.FoldoutStyle);
            if (HotReloadPrefs.ShowLogin)
            {
                ...
                RenderLicenseStatusInfo(currentState, loginStatus, allowHide, verbose);

                RenderLicenseInnerPanel(currentState, overrideActionButton);
        ...
        {
            HotReloadPrefs.ShowPromoCodes = EditorGUILayout.Foldout(HotReloadPrefs.ShowPromoCodes, "Promo Codes", true,
                HotReloadWindowStyles.FoldoutStyle);
            if (!HotReloadPrefs.ShowPromoCodes) return;
            if (promoCodeActivatedThisSession)
            {
                EditorGUILayout.HelpBox(
                    "Your promo code has been successfully activated. Free trial has been extended by 3 months.",
                    MessageType.Info);
            }
            else
            {
                ...
                {
                    if (GUILayout.Button("Activate promo code", bigButtonHeight)) RequestActivatePromoCode().Forget();
                }
            }
        ...
        private PromoCodeErrorType ToErrorType(string error)
        {
            ...
            if (error.Contains("Updating License Failed with error")) return PromoCodeErrorType.UPDATING_LICENSE_FAILED;

            if (error.Contains("Unknown exception")) return PromoCodeErrorType.UNKNOWN_EXCEPTION;

            if (error.Contains("Unsupported path")) return PromoCodeErrorType.UNSUPPORTED_PATH;
        ...
        {
            var defaultMsg =
                "We apologize, an error happened while activating your promo code. Please reach out to customer support for assistance.";
            switch (errorType)
            {
                ...
                case PromoCodeErrorType.LICENSE_NOT_TRIAL:
                    return defaultMsg;
                case PromoCodeErrorType.PROMO_CODE_NOT_FOUND:
                    return "Your promo code is invalid. Please ensure that you have entered the correct promo code.";
                ...
                case PromoCodeErrorType.LICENSE_ALREADY_EXTENDED:
                    return
                        "Your license has already been activated with a promo code. Only one promo code activation per license is allowed.";
                case PromoCodeErrorType.CONDITIONAL_CHECK_FAILED:
                    return
                        "We encountered an error while activating your promo code. Please try again. If the issue persists, please contact our customer support team for assistance.";
                case PromoCodeErrorType.NONE:
                    return
                        "There is an issue connecting to our servers. Please check your internet connection or contact customer support if the issue persists.";
        ...
        internal static void RenderLicenseInnerPanel(HotReloadRunTabState currentState,
            string overrideActionButton = null, bool renderLogout = true)
        {
            EditorGUILayout.LabelField("Email");
            GUI.SetNextControlName("email");
            _pendingEmail =
                EditorGUILayout.TextField(string.IsNullOrEmpty(_pendingEmail)
                    ? HotReloadPrefs.LicenseEmail
                    : _pendingEmail);
            _pendingEmail = _pendingEmail.Trim();

            EditorGUILayout.LabelField("Password");
            GUI.SetNextControlName("password");
            _pendingPassword = EditorGUILayout.PasswordField(string.IsNullOrEmpty(_pendingPassword)
                ? HotReloadPrefs.LicensePassword
                : _pendingPassword);
            ...
            using (new EditorGUI.DisabledScope(currentState.requestingLoginInfo))
            {
                ...
                if (string.IsNullOrEmpty(overrideActionButton)) btnLabel = "Login";
                using (new EditorGUILayout.HorizontalScope())
                {
                       ...
                       )
                    {
                        ...
                        {
                            _activateInfoMessage =
                                new Tuple<string, MessageType>("Please enter your password.", MessageType.Warning);
                        }
                        else
                        {
                            ...
                            if (!EditorCodePatcher.RequestingDownloadAndRun && !EditorCodePatcher.Running)
                                LoginOnDownloadAndRun(new LoginData(_pendingEmail, _pendingPassword)).Forget();
                            else
                                EditorCodePatcher.RequestLogin(_pendingEmail, _pendingPassword).Forget();
                        }
                    }

                    if (renderLogout) RenderLogout(currentState);
                }
            }
        ...
        {
            if (string.IsNullOrEmpty(email)) return "Please enter your email address.";

            if (!EditorWindowHelper.IsValidEmailAddress(email)) return "Please enter a valid email address.";

            if (email.Contains("+"))
                return
                    "Mail extensions (in a form of 'username+suffix@example.com') are not supported yet. Please provide your original email address (such as 'username@example.com' without '+suffix' part) as we're working on resolving this issue.";
            return null;
        }

        public static void RenderLogout(HotReloadRunTabState currentState)
        {
            if (currentState.loginStatus?.isLicensed != true) return;
        ...
        private static async Task LoginOnDownloadAndRun(LoginData loginData = null)
        {
        ...
        private static async Task LogoutOnDownloadAndRun()
        {
        ...
        private static async Task RequestLogout()
        {
            var i = 0;
            ...
            if (!EditorCodePatcher.RequestingLoginInfo && resp != null) EditorCodePatcher.HandleStatus(resp);
        }

        private static void RenderSwitchAuthMode()
        {
            var color = EditorGUIUtility.isProSkin
                ? new Color32(0x3F, 0x9F, 0xFF, 0xFF)
                : new Color32(0x0F, 0x52, 0xD7, 0xFF);
            if (HotReloadGUIHelper.LinkLabel("Forgot password?", 12, FontStyle.Normal, TextAnchor.MiddleLeft, color))
                if (EditorUtility.DisplayDialog("Recover password",
                        "Use company code 'naughtycult' and the email you signed up with in order to recover your account.",
        ...
        {
            if (currentState.downloadRequired && !currentState.downloadStarted) return;
                ...
                try
                {
                    ...
                    if (currentState.downloadRequired)
                    {
                        ...
                        using (new EditorGUILayout.HorizontalScope())
                        {
                            ...
                            if (GUI.Button(
                                    new Rect(barRect)
                                        { x = barRect.x + barRect.width + 5, height = barRect.height, width = 60 },
                                    new GUIContent(" Info", GUIHelper.GetLocalIcon("alert_info"))))
                                Application.OpenURL(Constants.AdditionalContentURL);
                        }
                    }
                    else
                    {
                        progress = EditorCodePatcher.Stopping
                            ? 1
                            : Mathf.Clamp(EditorCodePatcher.StartupProgress?.Item1 ?? 0f, 0f, 1f);
        ...
        private Texture2D MakeTexture(float maxHue)
        {
            ...
            for (var y = 0; y < height; y++)
            {
                ...
                for (var x = 0; x < width; x++) pix[y * width + x] = new Color(0.1f, (float)green, 0.1f, 1.0f);
            }
    ...
    {
        public static bool LinkLabel(string labelText, int fontSize, FontStyle fontStyle, TextAnchor alignment,
            Color? color = null)
        {
