...
namespace SingularityGroup.HotReload.Editor
{
    ...
    internal class RedeemLicenseHelper
    {
        ...
        private const string unknownError =
            "We apologize, an error happened while redeeming your license. Please reach out to customer support for assistance.";

        private const string redeemUrl = "https://vmhzj6jonn3qy7hk7tx7levpli0bstpj.lambda-url.us-east-1.on.aws/redeem";
        public static readonly RedeemLicenseHelper I = new();
        ...
        private readonly GUILayoutOption[] secondaryButtonLayoutOptions = { GUILayout.MaxWidth(100) };

        private string status;

        public RedeemLicenseHelper()
        {
            if (File.Exists(registerFlagPath)) RedeemStage = RedeemStage.Registration;
            try
            {
        ...
        public void RenderStage(HotReloadRunTabState state)
        {
            if (state.redeemStage == RedeemStage.Registration)
                ...
                RenderRedeem();
            else if (state.redeemStage == RedeemStage.Login) RenderLogin(state);
        }
        ...
        {
            RequestHelper.RequestEditorEvent(
                new Stat(StatSource.Client, StatLevel.Debug, StatFeature.Licensing, StatEventType.Register),
                new EditorExtraData { { StatKey.CompanySize, companySize } });
            if (companySize > 10)
            {
        ...
        private void RenderRedeem()
        {
            if (error != null)
                ...
                EditorGUILayout.HelpBox(
                    "To enable us to verify your purchase, please enter your invoice number/order ID. Additionally, provide the email address that you intend to use for managing your credentials.",
                    MessageType.Info);
            EditorGUILayout.Space();
            EditorGUILayout.Space();
            ...
            _pendingInvoiceNumber = EditorGUILayout
                .TextField(_pendingInvoiceNumber ?? HotReloadPrefs.RedeemLicenseInvoice)?.Trim();
            EditorGUILayout.Space();
            ...
            using (new EditorGUI.DisabledScope(requestingRedeem))
            {
                if (GUILayout.Button("Redeem", HotReloadRunTab.bigButtonHeight))
                    RedeemLicense(_pendingRedeemEmail, _pendingInvoiceNumber).Forget();
            }
            ...
            using (new EditorGUILayout.HorizontalScope())
            {
                ...
                if (GUILayout.Button("Skip", secondaryButtonLayoutOptions)) SwitchToStage(RedeemStage.Login);
                GUILayout.FlexibleSpace();
            }
        }
        ...
        private async Task RedeemLicense(string email, string invoiceNumber)
        {
            ...
            var resp = await RequestRedeem(email, invoiceNumber);
            status = resp?.status;
            if (status != null)
            {
        ...
        private string GetPrettyError(RedeemResponse response)
        {
            ...
            if (err.Contains("Invalid email")) return "Please enter a valid email address.";

            if (err.Contains("License invoice already redeemed"))
                return
                    "The invoice number/order ID you're trying to use has already been applied to redeem a license. Please enter a different invoice number/order ID. If you have already redeemed a license for another email, you may proceed to the next step.";

            if (err.Contains("Different license already redeemed by given email"))
                return
                    "The provided email has already been used to redeem a license. If you have previously redeemed a license, you can proceed to the next step and use your existing credentials. If not, please input a different email address.";

            if (err.Contains("Invoice not found"))
                return
                    "The invoice was not found. Please ensure that you've entered the correct invoice number/order ID.";

            if (err.Contains("Invoice refunded"))
        ...
        private async Task<RedeemResponse> RequestRedeem(string email, string invoiceNumber)
        {
            ...
            try
            {
                ...
                using (var resp = await redeemClient.PostAsync(redeemUrl, content, HotReloadWindow.Current.cancelToken)
                           .ConfigureAwait(false))
                {
                    if (resp.StatusCode != HttpStatusCode.OK)
                        return new RedeemResponse(null,
                            $"Redeem request failed. Status code: {(int)resp.StatusCode}, reason: {resp.ReasonPhrase}");
                    ...
                    {
                        return new RedeemResponse(null,
                            $"Failed deserializing redeem response with exception: {ex.GetType().Name}: {ex.Message}");
                    }
                }
            }
        ...
        private void RenderLogin(HotReloadRunTabState state)
        {
            if (status == statusSuccess)
                EditorGUILayout.HelpBox(
                    "Success! You will receive an email containing your license password shortly. Once you receive it, please enter the received password in the designated field below to complete your registration.",
                    MessageType.Info);
            else if (status == statusAlreadyClaimed)
                EditorGUILayout.HelpBox(
                    "Your license has already been redeemed. Please enter your existing password below.",
                    MessageType.Info);
            EditorGUILayout.Space();
            EditorGUILayout.Space();
            ...
            {
                GUILayout.FlexibleSpace();
                if (GUILayout.Button("Go Back", secondaryButtonLayoutOptions)) SwitchToStage(RedeemStage.Redeem);
                GUILayout.FlexibleSpace();
            }
        }
