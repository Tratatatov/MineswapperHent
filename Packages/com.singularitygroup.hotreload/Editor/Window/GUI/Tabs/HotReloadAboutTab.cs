...
namespace SingularityGroup.HotReload.Editor
{
    ...
    {
        internal static readonly OpenURLButton seeMore = new("See More", Constants.ChangelogURL);

        internal static readonly OpenDialogueButton manageLicenseButton = new("Manage License",
            Constants.ManageLicenseURL, "Manage License",
            "Upgrade/downgrade/edit your subscription and edit payment info.", "Open in browser", "Cancel");

        internal static readonly OpenDialogueButton manageAccountButton = new("Manage Account",
            Constants.ManageAccountURL, "Manage Account",
            "Login with company code 'naughtycult'. Use the email you signed up with. Your initial password was sent to you by email.",
            "Open in browser", "Cancel");

        internal static readonly OpenURLButton contactButton = new("Contact", Constants.ContactURL);
        internal static readonly OpenURLButton discordButton = new("Join Discord", Constants.DiscordInviteUrl);

        internal static readonly OpenDialogueButton reportIssueButton = new("Report issue", Constants.ReportIssueURL,
            "Report issue",
            "Report issue in our public issue tracker. Requires gitlab.com account (if you don't have one and are not willing to make it, please contact us by other means such as our website).",
            "Open in browser", "Cancel");
        ...
        private readonly string _changelogDir = Path.Combine(PackageConst.LibraryCachePath, "changelog.json");
        ...
        public HotReloadAboutTab(HotReloadWindow window) : base(window, "Help", "_Help",
            "Info and support for Hot Reload for Unity.")
        {
        }
        ...
        private async Task FetchChangelog()
        {
            ...
            if (_requestedChangelog || LatestChangelogLoaded(_changelog)) return;
        ...
        private void RenderChangelog()
        {
            ...
            using (new EditorGUILayout.HorizontalScope(HotReloadWindowStyles.SectionInnerBoxWide))
            {
                ...
                {
                    HotReloadPrefs.ShowChangeLog = EditorGUILayout.Foldout(HotReloadPrefs.ShowChangeLog, "Changelog",
                        true, HotReloadWindowStyles.FoldoutStyle);
                    if (!HotReloadPrefs.ShowChangeLog) return;
                    // changelog versions                        
                    var maxChangeLogs = 5;
                    var index = 0;
        ...
        public override void OnGUI()
        {
            ...
            if (Event.current.type == EventType.Layout)
                currentState = new HotReloadAboutTabState(
                    ...
                    _window.RunTab.TrialLicense,
                    _window.RunTab.HasPayedLicense
                );
            using (var scope = new EditorGUILayout.ScrollViewScope(_aboutTabScrollPos, GUI.skin.horizontalScrollbar,
        ...
        private void RenderHelpButtons()
        {
            ...
            using (new EditorGUILayout.HorizontalScope())
            {
                using (new EditorGUILayout.VerticalScope())
                {
                    ...
                    OpenURLButton.RenderRaw(bigButtonRect, "Documentation", Constants.DocumentationURL,
                        HotReloadWindowStyles.HelpTabButton);
                    ...
                    using (new EditorGUILayout.HorizontalScope())
                    {
                        OpenURLButton.RenderRaw(
                            new Rect
                            {
                                x = firstLayerX, y = firstLayerY, width = firstLayerWidth, height = buttonHeight
                            }, contactButton.text, contactButton.url, HotReloadWindowStyles.HelpTabButton);
                        OpenURLButton.RenderRaw(
                            new Rect
                            {
                                x = secondLayerX, y = firstLayerY, width = secondLayerWidth, height = buttonHeight
                            }, "Unity Forum", Constants.ForumURL, HotReloadWindowStyles.HelpTabButton);
                    }

                    using (new EditorGUILayout.HorizontalScope())
                    {
                        OpenDialogueButton.RenderRaw(
                            new Rect
                            {
                                x = firstLayerX, y = secondLayerY, width = firstLayerWidth, height = buttonHeight
                            }, reportIssueButton.text, reportIssueButton.url, reportIssueButton.title,
                            reportIssueButton.message, reportIssueButton.ok, reportIssueButton.cancel,
                            HotReloadWindowStyles.HelpTabButton);
                        OpenURLButton.RenderRaw(
                            new Rect
                            {
                                x = secondLayerX, y = secondLayerY, width = secondLayerWidth, height = buttonHeight
                            }, discordButton.text, discordButton.url, HotReloadWindowStyles.HelpTabButton);
                    }
                }
            }
        ...
        private void RenderLogButtons()
        {
            if (currentState.logsFodlerExists)
            {
                ...
                if (GUILayout.Button("Browse all logs")) Process.Start($"\"{logsPath}\"");
