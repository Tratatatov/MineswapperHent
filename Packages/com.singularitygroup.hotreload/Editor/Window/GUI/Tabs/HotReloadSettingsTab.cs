...
namespace SingularityGroup.HotReload.Editor
{
    ...
    internal class HotReloadSettingsTab : HotReloadTabBase
    {
        ...
        public static readonly IOption[] allOptions =
        {
        ...
        private readonly Lazy<BuildTargetGroup> currentBuildTarget =
            new(() => BuildPipeline.GetBuildTargetGroup(EditorUserBuildSettings.activeBuildTarget));

        private readonly Lazy<bool> isCurrentBuildTargetSupported = new(() =>
        {
        ...
        public override void OnGUI()
        {
            ...
            if (Event.current.type == EventType.Layout)
                currentState = new HotReloadSettingsTabState(
                    ...
                    EditorCodePatcher.Status != null && EditorCodePatcher.Status?.isTrial == true,
                    EditorCodePatcher.Status,
                    ServerHealthCheck.I.IsServerHealthy,
                    RedeemLicenseHelper.I.RegistrationRequired
                );
            using (var scope = new EditorGUILayout.ScrollViewScope(_settingsTabScrollPos, GUI.skin.horizontalScrollbar,
                ...
                using (new EditorGUILayout.VerticalScope(HotReloadWindowStyles.DynamicSectionHelpTab))
                {
                    ...
                    using (new EditorGUILayout.HorizontalScope(HotReloadWindowStyles.SectionOuterBoxCompact))
                    {
                        using (new EditorGUILayout.HorizontalScope(HotReloadWindowStyles.SectionInnerBoxWide))
                        {
                            using (new EditorGUILayout.VerticalScope())
                            {
                                HotReloadPrefs.ShowConfiguration = EditorGUILayout.Foldout(
                                    HotReloadPrefs.ShowConfiguration, "Configuration", true,
                                ...
                                if (HotReloadPrefs.ShowConfiguration)
                                {
                                    ...
                                    {
                                        using (new EditorGUILayout.VerticalScope(paddedStyle ??
                                                   (paddedStyle = new GUIStyle
                                    ...
                                    using (new EditorGUI.DisabledScope(!EditorCodePatcher
                                               .autoRecompileUnsupportedChangesSupported))
                                    {
                                        RenderAutoRecompileUnsupportedChanges();
                                        if (HotReloadPrefs.AutoRecompileUnsupportedChanges &&
                                            EditorCodePatcher.autoRecompileUnsupportedChangesSupported)
                                            using (new EditorGUILayout.VerticalScope(paddedStyle ??
                                                       (paddedStyle = new GUIStyle
                                                           { padding = new RectOffset(20, 0, 0, 0) })))
                                            {
        ...
        {
            var newSettings = EditorGUILayout.BeginToggleGroup(
                new GUIContent("Allow to manage Unity's Auto Compile settings (recommended)"),
                HotReloadPrefs.AllowDisableUnityAutoRefresh);
            ...
            if (HotReloadPrefs.AllowDisableUnityAutoRefresh)
                toggleDescription =
                    "Hot Reload will manage Unity's Auto Refresh and Script Compilation settings when it's running. Previous settings will be restored when Hot Reload is stopped.";
            else
                toggleDescription =
                    "Enable to allow Hot Reload to manage Unity's Auto Refresh and Script Compilation settings when it's running. If enabled, previous settings will be restored when Hot Reload is stopped.";
        ...
        {
            var newSettings = EditorGUILayout.BeginToggleGroup(new GUIContent("Asset refresh (recommended)"),
                HotReloadPrefs.AllAssetChanges);
            if (newSettings != HotReloadPrefs.AllAssetChanges)
            {
                ...
                if (ServerHealthCheck.I.IsServerHealthy)
                {
                    ...
                    if (restartServer) EditorCodePatcher.RestartCodePatcher().Forget();
                }
            }
            ...
            if (HotReloadPrefs.AllAssetChanges)
                ...
                toggleDescription =
                    "Enable to allow Hot Reload to refresh changed assets in the project. All asset types are supported including sprites, prefabs, shaders etc.";
            EditorGUILayout.LabelField(toggleDescription, HotReloadWindowStyles.WrapStyle);
            EditorGUILayout.EndToggleGroup();
            EditorGUILayout.Space(3f);
        }
        ...
        {
            HotReloadPrefs.IncludeShaderChanges = EditorGUILayout.BeginToggleGroup(new GUIContent("Refresh shaders"),
                HotReloadPrefs.IncludeShaderChanges);
            string toggleDescription;
            if (HotReloadPrefs.IncludeShaderChanges)
                toggleDescription =
                    "Hot Reload will auto refresh shaders. Note that enabling this setting might impact performance.";
            else
                toggleDescription =
                    "Enable to auto-refresh shaders. Note that enabling this setting might impact performance";
            EditorGUILayout.LabelField(toggleDescription, HotReloadWindowStyles.WrapStyle);
            EditorGUILayout.EndToggleGroup();
        }
        ...
        {
            if (!HotReloadCli.CanOpenInBackground) return;
            var newSettings = EditorGUILayout.BeginToggleGroup(new GUIContent("Hide console window on start"),
            ...
            if (HotReloadPrefs.DisableConsoleWindow)
                toggleDescription =
                    "Hot Reload will start without creating a console window. Logs can be accessed through \"Help\" tab.";
            else
        ...
        {
            var newSettings = EditorGUILayout.BeginToggleGroup(new GUIContent("Autostart on Unity open"),
                HotReloadPrefs.LaunchOnEditorStart);
            if (newSettings != HotReloadPrefs.LaunchOnEditorStart) HotReloadPrefs.LaunchOnEditorStart = newSettings;
            string toggleDescription;
        ...
        private void RenderShowPatchingNotifications()
        {
            HotReloadPrefs.ShowPatchingNotifications =
                EditorGUILayout.BeginToggleGroup(new GUIContent("Patching Indication"),
                    HotReloadPrefs.ShowPatchingNotifications);
            string toggleDescription;
            if (!EditorWindowHelper.supportsNotifications)
        ...
        {
            HotReloadPrefs.ShowCompilingUnsupportedNotifications = EditorGUILayout.BeginToggleGroup(
                new GUIContent("Compiling Unsupported Changes Indication"),
                HotReloadPrefs.ShowCompilingUnsupportedNotifications);
            string toggleDescription;
            if (!EditorWindowHelper.supportsNotifications)
                toggleDescription =
                    "Compiling Unsupported Changes Notification is not supported in the Unity version you use.";
            else if (!HotReloadPrefs.ShowCompilingUnsupportedNotifications)
                toggleDescription =
                    "Enable to show GameView and SceneView indications when compiling unsupported changes.";
            else
                toggleDescription =
                    "Indications will be shown in GameView and SceneView when compiling unsupported changes.";
            EditorGUILayout.LabelField(toggleDescription, HotReloadWindowStyles.WrapStyle);
            EditorGUILayout.EndToggleGroup();
        }
        ...
        {
            HotReloadPrefs.AutoRecompileUnsupportedChanges = EditorGUILayout.BeginToggleGroup(
                new GUIContent("Auto recompile unsupported changes (recommended)"),
                HotReloadPrefs.AutoRecompileUnsupportedChanges &&
                EditorCodePatcher.autoRecompileUnsupportedChangesSupported);
            string toggleDescription;
            if (!EditorCodePatcher.autoRecompileUnsupportedChangesSupported)
                toggleDescription =
                    "Auto recompiling unsupported changes is not supported in the Unity version you use.";
        ...
        {
            HotReloadPrefs.AutoRecompilePartiallyUnsupportedChanges = EditorGUILayout.BeginToggleGroup(
                new GUIContent("Include partially unsupported changes"),
                HotReloadPrefs.AutoRecompilePartiallyUnsupportedChanges);
            string toggleDescription;
        ...
        {
            HotReloadPrefs.AutoRecompileUnsupportedChangesImmediately = EditorGUILayout.BeginToggleGroup(
                new GUIContent("Recompile immediately"), HotReloadPrefs.AutoRecompileUnsupportedChangesImmediately);
            string toggleDescription;
            if (HotReloadPrefs.AutoRecompileUnsupportedChangesImmediately)
                ...
                toggleDescription =
                    "Unsupported changes will be recompiled when editor is focused. Enable to recompile immediately.";
            EditorGUILayout.LabelField(toggleDescription, HotReloadWindowStyles.WrapStyle);
            EditorGUILayout.EndToggleGroup();
        }
        ...
        {
            HotReloadPrefs.AutoRecompileUnsupportedChangesInPlayMode = EditorGUILayout.BeginToggleGroup(
                new GUIContent("Recompile in Play Mode"), HotReloadPrefs.AutoRecompileUnsupportedChangesInPlayMode);
            string toggleDescription;
        ...
        {
            HotReloadPrefs.AutoRecompileUnsupportedChangesOnExitPlayMode = EditorGUILayout.BeginToggleGroup(
                new GUIContent("Recompile on exit Play Mode"),
                HotReloadPrefs.AutoRecompileUnsupportedChangesOnExitPlayMode);
            string toggleDescription;
        ...
        {
            HotReloadPrefs.ShowOnDevice = EditorGUILayout.Foldout(HotReloadPrefs.ShowOnDevice, "On-Device", true,
                HotReloadWindowStyles.FoldoutStyle);
            if (!HotReloadPrefs.ShowOnDevice) return;
...

            {
                ...
                if (string.IsNullOrEmpty(ip))
                    text =
                        $"If auto-pair fails, find your local IP in OS settings, and use this format to connect: '{{ip}}:{RequestHelper.port}'";
                else
            ...
            // if you build for Android now, will Hot Reload work?
            {
                ...
                if (GUILayout.Button("Fix All", GUILayout.MaxWidth(90f))) FixAllUnsupportedSettings(so);
            ...
            // Settings checkboxes (Hot Reload options)
            {
                ...
                if (settingsObject) optionsSection.DrawGUI(so);
            }
        ...
        private void FixAllUnsupportedSettings(SerializedObject so)
        {
            ...
            if (!EditorUserBuildSettings.development) EditorUserBuildSettings.development = true;
        ...
        public static bool TrySwitchToStandalone()
        {
            ...
            if (current == buildTarget) return true;
        ...
        public void DrawSectionCheckBuildSupport(SerializedObject so)
        {
            ...
            using (new EditorGUI.DisabledScope(!isCurrentBuildTargetSupported.Value))
            {
                ...
                DrawHorizontalCheck(StrippingLevel == ManagedStrippingLevel.Disabled,
                    ...
                    $"Stripping Level = {StrippingLevel}",
                    "Code stripping needs to be disabled to ensure that all methods are available for patching."
                );
            }
        }
        ...
        private void DrawHorizontalCheck(bool condition, string okText, string notOkText = null,
            string suggestedSolutionText = null, bool hasFix = true)
        {
            if (okText == null) throw new ArgumentNullException(nameof(okText));
            if (notOkText == null) notOkText = okText;
            ...
            if (!condition && hasFix) isSupported = false;
        ...
        private void DrawDisabledCircle(Rect rect)
        {
            DrawCircleIcon(rect,
                Resources.Load<Texture>("icon_circle_gray"),
                Color.clear);
            // smaller circle draws less attention
        }

        private void DrawBlackCircle(Rect rect)
        {
            DrawCircleIcon(rect,
                Resources.Load<Texture>("icon_circle_black"),
                new Color(0.14f, 0.14f, 0.14f));
            // black is too dark in unity light theme
        }
        ...
        private void DrawCircleIcon(Rect rect, Texture circleIcon, Color borderColor)
        {
            ...
            if (circleIcon != null) GUI.DrawTexture(rect, circleIcon, ScaleMode.ScaleToFit);
