...
namespace SingularityGroup.HotReload.Editor.ProjectGeneration
{
    ...
    private const string k_TargetLanguageVersion = "latest";
        ...
        public static string GetUnityProjectDirectory(string dataPath)
        {
            return new DirectoryInfo(dataPath).Parent.FullName;
        }

        public static string GetSolutionFilePath(string dataPath)
        {
            return Path.Combine(tempDir, Path.GetFileName(GetUnityProjectDirectory(dataPath)) + ".sln");
        }

        public static Task GenerateSlnAndCsprojFiles(string dataPath)
        {
            if (!IsSyncing) return GenerateAsync(dataPath);
            return Task.CompletedTask;
        }

        public static Task EnsureSlnAndCsprojFiles(string dataPath)
        {
            if (File.Exists(GetSolutionFilePath(dataPath))) return Task.CompletedTask;
        ...
        public async Task Sync()
        {
            ...
            if (config.useBuiltInProjectGeneration) return;
        ...
        private bool ShouldFileBePartOfSolution(string file)
        {
            ...
            if (IsInternalizedPackagePath(file)) return false;
        ...
        private async Task GenerateAndWriteSolutionAndProjects(Config config)
        {
            ...
            var projectExclusionRegex = config.projectExclusionRegex != null
                ? new Regex(config.projectExclusionRegex, RegexOptions.Compiled | RegexOptions.Singleline)
                : null;
...
# else
                    projectPath =
 ProjectFile(part).Replace('\\', '/').Replace(m_ProjectDirectory.Replace('\\', '/'), "");
#endif
        ...
        private void SyncFileIfNotChanged(string path, string newContents)
        {
            try
            {
                if (m_FileIOProvider.Exists(path) && newContents == m_FileIOProvider.ReadAllText(path)) return;
            }
        ...
        private string ProjectText(ProjectPart assembly, List<ResponseFileData> responseFilesData,
            HashSet<string> filteredProjects, string[] runtimeDependencies, Config config)
        {
            ...
            var internalAssemblyReferences = assembly.AssemblyReferences
                .Where(reference => filteredProjects.Contains(reference.name) ||
                                    !reference.sourceFiles.Any(ShouldFileBePartOfSolution)).Select(i => i.outputPath);
            var allReferences =
            ...
            {
                var fullReference = Path.IsPathRooted(reference)
                    ? reference
                    : Path.Combine(m_ProjectDirectory, reference);
                AppendReference(fullReference, projectBuilder);
            }

            foreach (var path in runtimeDependencies) AppendReference(path, projectBuilder);
            ...
            if (assembly.AssemblyReferences.Length > 0)
            {
                ...
                foreach (var reference in assembly.AssemblyReferences.Where(i =>
                             !filteredProjects.Contains(i.name) && i.sourceFiles.Any(ShouldFileBePartOfSolution)))
                {
                    var name = GetProjectName(reference.name, reference.defines);
                    projectBuilder.Append("    <ProjectReference Include=\"").Append(name).Append(".csproj")
                        .Append("\">")
                        .Append(Environment.NewLine);
                    projectBuilder.Append("      <Project>{").Append(ProjectGuid(name)).Append("}</Project>")
                        .Append(Environment.NewLine);
        ...
        )
        {
            ...
            var arguments = new object[]
            {
                ...
                string.Join(";",
                    assembly.Defines.Concat(responseFilesData.SelectMany(x => x.Defines)).Distinct().ToArray()),
                MSBuildNamespaceUri,
                assembly.Name,
                assembly.OutputPath,
                ...
                config.excludeAllAnalyzers
                    ? ""
                    : GenerateAnalyserAdditionalFiles(otherResponseFilesData["additionalfile"]
                        .SelectMany(x => x.Split(';')).Distinct().ToArray()),
        ...
        private string GenerateRoslynAnalyzerRulesetPath(ProjectPart assembly,
            ILookup<string, string> otherResponseFilesData)
        {
#if UNITY_2020_2_OR_NEWER
            return GenerateAnalyserRuleSet(otherResponseFilesData["ruleset"]
                .Append(assembly.CompilerOptions.RoslynAnalyzerRulesetPath)
                .Where(a => !string.IsNullOrEmpty(a)).Distinct().Select(x => MakeAbsolutePath(x).NormalizePath())
                .ToArray());
#else
        ...
        private static string GenerateDocumentationFile(string[] paths)
        {
            ...
            return
                $"{Environment.NewLine}{string.Join(Environment.NewLine, paths.Select(a => $"  <DocumentationFile>{a}</DocumentationFile>"))}";
        }

        private static string GenerateWarningAsError(IEnumerable<string> args, IEnumerable<string> argsMinus,
            IEnumerable<string> argsPlus)
        {
            ...
            if (warningIds.Any())
                returnValue +=
                    $"{Environment.NewLine}    <WarningsAsErrors>{string.Join(";", warningIds)}</WarningsAsErrors>";

            if (argsMinus.Any())
                returnValue +=
                    $"{Environment.NewLine}    <WarningsNotAsErrors>{string.Join(";", argsMinus)}</WarningsNotAsErrors>";
        ...
        private string SolutionText(ProjectPart[] islands)
        {
            ...
            var projectConfigurations = string.Join(Environment.NewLine,
                islands.Select(i => GetProjectActiveConfigurations(ProjectGuid(GetProjectName(i.Name, i.Defines))))
                    .ToArray());
                    ...
                    return x.OtherArguments
                        ...
                        .Select(b =>
                        {
                            ...
                            if (b.Substring(1).StartsWith(warnaserror, StringComparison.Ordinal))
                                return new KeyValuePair<string, string>(warnaserror,
                                    b.Substring(warnaserror.Length + 1));
        ...
        private static string GenerateAnalyserAdditionalFiles(string[] paths)
        {
            ...
            var analyserBuilder = new StringBuilder();
            analyserBuilder.AppendLine("  <ItemGroup>");
            foreach (var path in paths) analyserBuilder.AppendLine($"    <AdditionalFiles Include=\"{path}\" />");
        ...
        public ProjectGenerationFlag ProjectGenerationFlag =>
            ProjectGenerationFlag.Local | ProjectGenerationFlag.Embedded;
        ...
        public PackageInfo FindForAssetPath(string assetPath)
        {
            ...
            if (m_PackageInfoCache.TryGetValue(parentPackageAssetPath, out cachedPackageInfo)) return cachedPackageInfo;
        ...
        private async Task BuildPackageInfoCache()
        {
            ...
            await Task.Run(() =>
            {
                ...
                {
                    if (string.IsNullOrWhiteSpace(m_AllAssetPaths[i])) continue;

                    var parentPackageAssetPath = ResolvePotentialParentPackageAssetPath(m_AllAssetPaths[i]);
                    if (parentPackageAssetPath == null) continue;
            ...
            {
                var result = PackageInfo.FindForAssetPath(parentAssetPath);
        ...
        private async Task BuildPostProcessors()
        {
            ...
            m_PostProcessors = await Task.Run(() =>
            {
                ...
                foreach (var type in types)
                    ...
                    {
                        Log.Warning("The type '{0}' was expected to have a public default constructor but it didn't",
                            type.FullName);
                    }
                    catch (TargetInvocationException ex)
                    {
                        Log.Warning("Exception occurred when invoking default constructor of '{0}':\n{1}",
                            type.FullName, ex.InnerException);
                    }
                    catch (Exception ex)
                    {
            ...
            foreach (var postProcessor in m_PostProcessors) postProcessor.InitializeOnMainThread();
#else
