    ...
    internal class PrebuildIncludeResources : IPreprocessBuild, IPostprocessBuild
    {
        ...
        public void OnPostprocessBuild(BuildTarget target, string path)
        {
            if (warnSettingsNotSupported)
                Debug.LogWarning(
                    "Hot Reload was not included in the build because one or more build settings were not supported.");
        }
        ...
        public void OnPreprocessBuild(BuildTarget target, string path)
        {
            try
            {
                ...
                else
                {
                    ...
                    if (IncludeInBuildOption.I.GetValue(so)) warnSettingsNotSupported = true;
                }
            }
