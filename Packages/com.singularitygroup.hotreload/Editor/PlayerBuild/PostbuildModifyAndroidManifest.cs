...
*/
        ...
        private static string FindAndroidManifest(string unityLibraryPath)
        {
            ...
            if (File.Exists(manifestFilePath)) return manifestFilePath;

            Log.Info("Did not find {0} at {1}, searching for manifest file inside {2}", manifestFileName,
                manifestFilePath, dir.FullName);
