...
namespace SingularityGroup.HotReload.Editor
{
    internal static class AssemblyOmission
    {
        ...
        {
            Log.Info(
                "To compile C# files same as a Player build, we must omit projects which aren't part of the selected Player build.");
            var omitted = GetOmittedProjects(EditorUserBuildSettings.activeScriptCompilationDefines);
            Log.Info("---------");

            foreach (var name in omitted) Log.Info("omitted editor/other project named: {0}", name);
        }

        // main thread only
        public static string[] GetOmittedProjects(string allDefineSymbols, bool verboseLogs = false)
        {
            if (Cache.ommitedProjects != null) return Cache.ommitedProjects;
        ...
        private static string[] GetOmittedProjects(string[] allDefineSymbols, bool verboseLogs = false)
        {
            ...
            if (verboseLogs)
            {
                foreach (var name in editorAssemblies) Log.Info("found project named {0}", name);
        ...
        internal static class DefineConstraints
        {
            ...
            private static readonly Dictionary<string, string> syntaxMap = new()
            {
                { "OR", "||" },
                { "AND", "&&" },
                { "NOT", "!" }
            ...
            public static string[] GetOmittedAssemblies(string[] defineSymbols)
            {
                ...
                foreach (var asmdefFile in asmdefFiles)
                {
                    ...
                    var allPass = asmdef.defineConstraints.All(constraint =>
                        EvaluateDefineConstraint(constraint, defineSymbols));
                    if (!allPass) shouldOmit.Add(asmdef.name);
                }
            ...
            public static bool EvaluateDefineConstraint(string input, string[] defineSymbols)
            {
                ...
                foreach (var token in tokens)
                    if (!syntaxMap.ContainsKey(token) && token != "false" && token != "true")
                    {
                        ...
                        input = input.Substring(0, index) + defineSymbols.Contains(token) +
                                input.Substring(index + token.Length);
                    }
