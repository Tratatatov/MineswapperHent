...
namespace SingularityGroup.HotReload.Editor
{
    ...
    internal class AlertData
    {
        ...
        public AlertData(AlertEntryType alertEntryType, DateTime createdAt, bool detiled = false,
            EntryType entryType = EntryType.Standalone, string errorString = null, string methodName = null,
            string methodSimpleName = null, PartiallySupportedChange partiallySupportedChange = default,
            string[] patchedMethodsDisplayNames = null)
        {
    ...
    internal class AlertEntry
    {
        ...
        internal AlertEntry(AlertType alertType, string title, string description, DateTime timestamp,
            string shortDescription = null, Action actionData = null, AlertType? iconType = null,
            EntryType entryType = EntryType.Standalone, AlertData alertData = default, bool hasExitButton = true)
        {
    ...
    internal static class HotReloadTimelineHelper
    {
        ...
        internal static readonly Dictionary<AlertType, string> alertIconString = new()
        {
        ...
        public static Dictionary<PartiallySupportedChange, string> partiallySupportedChangeDescriptions = new()
        {
            {
                PartiallySupportedChange.LambdaClosure,
                "A lambda closure was edited (captured variable was added or removed). Changes to it will only be visible to the next created lambda(s)."
            },
            {
                PartiallySupportedChange.EditAsyncMethod,
                "An async method was edited. Changes to it will only be visible the next time this method is called."
            },
            {
                PartiallySupportedChange.AddMonobehaviourMethod,
                "A new method was added. It will not show up in the Inspector until the next full recompilation."
            },
            {
                PartiallySupportedChange.EditMonobehaviourField,
                "A field in a MonoBehaviour was removed or reordered. The inspector will not notice this change until the next full recompilation."
            },
            {
                PartiallySupportedChange.EditCoroutine,
                "An IEnumerator/IEnumerable was edited. When used as a coroutine, changes to it will only be visible the next time the coroutine is created."
            },
            {
                PartiallySupportedChange.AddEnumMember,
                "An enum member was added. ToString and other reflection methods work only after the next full recompilation. Additionally, changes to the enum order may not apply until you patch usages in other places of the code."
            },
            {
                PartiallySupportedChange.EditFieldInitializer,
                "A field initializer was edited. Changes will only apply to new instances of that type, since the initializer for an object only runs when it is created."
            },
            {
                PartiallySupportedChange.AddMethodWithAttributes,
                "A method with attributes was added. Method attributes will not have any effect until the next full recompilation."
            },
            {
                PartiallySupportedChange.GenericMethodInGenericClass,
                "A generic method was edited. Usages in non-generic classes applied, but usages in the generic classes are not supported."
            }
        };

        internal static List<AlertEntry> Suggestions = new();

        private static readonly Regex shortDescriptionRegex = new(@"^(\w+)\s(\w+)(?=:)", RegexOptions.Compiled);

        internal static List<AlertEntry> expandedEntries = new();

        private static float maxScrollPos;
        internal static List<AlertEntry> EventsTimeline { get; private set; } = new();

        internal static int UnsupportedChangesCount => EventsTimeline.Count(alert =>
            alert.alertType == AlertType.UnsupportedChange && alert.entryType != EntryType.Child);

        internal static int PartiallySupportedChangesCount => EventsTimeline.Count(alert =>
            alert.alertType == AlertType.PartiallySupportedChange && alert.entryType != EntryType.Child);

        internal static int UndetectedChangesCount => EventsTimeline.Count(alert =>
            alert.alertType == AlertType.UndetectedChange && alert.entryType != EntryType.Child);

        internal static int CompileErrorsCount =>
            EventsTimeline.Count(alert => alert.alertType == AlertType.CompileError);

        internal static int AppliedChangesCount =>
            EventsTimeline.Count(alert => alert.alertType == AlertType.AppliedChange);
        ...
        public static void InitPersistedEvents()
        {
            ...
            try
            {
                ...
                for (var i = persistedAlertData.alertDatas.Length - 1; i >= 0; i--)
                {
                    var alertData = persistedAlertData.alertDatas[i];
                    switch (alertData.alertEntryType)
                    {
                        case AlertEntryType.Error:
                            CreateErrorEventEntry(alertData.errorString, alertData.entryType, alertData.createdAt);
                            break;
                        case AlertEntryType.Failure:
                            if (alertData.entryType == EntryType.Parent)
                                CreateReloadFinishedWithWarningsEventEntry(alertData.createdAt,
                                    alertData.patchedMethodsDisplayNames);
                            else
                                CreatePatchFailureEventEntry(alertData.errorString, alertData.methodName,
                                    alertData.methodSimpleName, alertData.entryType, alertData.createdAt);
                            break;
                        ...
                        case AlertEntryType.PartiallySupportedChange:
                            if (alertData.entryType == EntryType.Parent)
                                CreateReloadPartiallyAppliedEventEntry(alertData.createdAt,
                                    alertData.patchedMethodsDisplayNames);
                            else
                                CreatePartiallyAppliedEventEntry(alertData.partiallySupportedChange,
                                    alertData.entryType, alertData.detiled, alertData.createdAt);
                            break;
                        case AlertEntryType.UndetectedChange:
                            CreateReloadUndetectedChangeEventEntry(alertData.createdAt);
                            break;
                    }
                }
            }
        ...
        internal static void PersistTimeline()
        {
            ...
            for (var i = 0; i < EventsTimeline.Count; i++) alertDatas[i] = EventsTimeline[i].alertData;
            var persistedData = new PersistedAlertData(alertDatas);
            try
            {
                File.WriteAllText(filePath, JsonConvert.SerializeObject(persistedData));
            }
            catch (Exception e)
            {
        ...
        {
            var total = 0;
            if (HotReloadPrefs.RunTabUnsupportedChangesFilter) total += UnsupportedChangesCount;
            if (HotReloadPrefs.RunTabPartiallyAppliedPatchesFilter) total += PartiallySupportedChangesCount;
            if (HotReloadPrefs.RunTabUndetectedPatchesFilter) total += UndetectedChangesCount;
            if (HotReloadPrefs.RunTabCompileErrorFilter) total += CompileErrorsCount;
            if (HotReloadPrefs.RunTabAppliedPatchesFilter) total += AppliedChangesCount;
            return total;
        }

        internal static void RenderCompileButton()
        {
            if (GUILayout.Button("Recompile", GUILayout.Width(80))) HotReloadRunTab.RecompileWithChecks();
        }

        internal static void RenderErrorEventActions(string description, ErrorData errorData)
        {
            var maxLen = 2400;
            var text = errorData.stacktrace;
            if (text.Length > maxLen) text = text.Substring(0, maxLen) + "...";

            GUILayout.TextArea(text, HotReloadWindowStyles.StacktraceTextAreaStyle);

            if (errorData.file || !errorData.stacktrace.Contains("error CS")) GUILayout.Space(10f);

            using (new EditorGUILayout.HorizontalScope())
            {
                if (!errorData.stacktrace.Contains("error CS")) RenderCompileButton();
        ...
        {
            if (count == 0) return GUIHelper.ConvertToGrayscale(alertIconString[alertType]);
        ...
        internal static void RenderAlertFilters()
        {
            using (new EditorGUILayout.HorizontalScope())
            {
                ...
                HotReloadPrefs.RunTabPartiallyAppliedPatchesFilter = GUILayout.Toggle(
                    ...
                    new GUIContent(text,
                        GetFilterIcon(PartiallySupportedChangesCount, AlertType.PartiallySupportedChange)),
                    HotReloadWindowStyles.EventFiltersStyle);
        ...
        internal static void CreateErrorEventEntry(string errorString, EntryType entryType = EntryType.Standalone,
            DateTime? createdAt = null)
        {
            ...
            var title = errorString.Contains("error CS")
                ? "Compile error"
                : "Unsupported change";
            var errorData = ErrorData.GetErrorData(errorString);
            var description = errorData.error;
            string shortDescription = null;
            if (alertType != AlertType.CompileError) shortDescription = shortDescriptionRegex.Match(description).Value;
            ...
            InsertEntry(new AlertEntry(
                ...
                entryType: entryType,
                alertData: new AlertData(AlertEntryType.Error, timestamp, errorString: errorString,
                    entryType: entryType)
            ));
        }

        internal static void CreatePatchFailureEventEntry(string errorString, string methodName,
            string methodSimpleName = null, EntryType entryType = EntryType.Standalone, DateTime? createdAt = null)
        {
            var timestamp = createdAt ?? DateTime.Now;
            var errorData = ErrorData.GetErrorData(errorString);
            ...
            InsertEntry(new AlertEntry(
                ...
                actionData: actionData,
                entryType: entryType,
                alertData: new AlertData(AlertEntryType.Failure, timestamp, errorString: errorString,
                    methodName: methodName, methodSimpleName: methodSimpleName, entryType: entryType)
            ));
        }

        public static T[] TruncateList<T>(T[] originalList, int len)
        {
            if (originalList.Length <= len) return originalList;
            // Create a new list with a maximum of 25 items
            var truncatedList = new T[len];

            for (var i = 0; i < originalList.Length && i < len; i++) truncatedList[i] = originalList[i];
            ...
            string[] patchedMethodsDisplayNames = null)
        {
            ...
            var patchesList = patchedMethodsDisplayNames?.Length > 0
            ...
            InsertEntry(entry);
            if (patchedMethodsDisplayNames?.Length > 0) expandedEntries.Add(entry);
        }
            ...
            string[] patchedMethodsDisplayNames = null)
        {
            ...
            var entry = new AlertEntry(
                ...
                description: patchedMethodsDisplayNames?.Length > 0
                    ? $"• {(truncated ? patchesList + "\n...\n\nSee unsupported changes below" : patchesList + "\n\nSee unsupported changes below")}"
                    : "See detailed entries below",
                entryType: EntryType.Parent,
                alertData: new AlertData(AlertEntryType.Failure, timestamp, entryType: EntryType.Parent,
                    patchedMethodsDisplayNames: patchedMethodsDisplayNames)
            );
            InsertEntry(entry);
            if (patchedMethodsDisplayNames?.Length > 0) expandedEntries.Add(entry);
            ...
            string[] patchedMethodsDisplayNames = null)
        {
            ...
            var entry = new AlertEntry(
                ...
                description: patchedMethodsDisplayNames?.Length > 0
                    ? $"• {(truncated ? patchesList + "\n...\n\nSee partially applied changes below" : patchesList + "\n\nSee partially applied changes below")}"
                    : "See detailed entries below",
                entryType: EntryType.Parent,
                alertData: new AlertData(AlertEntryType.PartiallySupportedChange, timestamp,
                    entryType: EntryType.Parent, patchedMethodsDisplayNames: patchedMethodsDisplayNames)
            );
            InsertEntry(entry);
            if (patchedMethodsDisplayNames?.Length > 0) expandedEntries.Add(entry);
        }
        ...
        internal static void CreateReloadUndetectedChangeEventEntry(DateTime? createdAt = null)
        {
            ...
            InsertEntry(new AlertEntry(
                ...
                description:
                "Code semantics didn't change (e.g. whitespace) or the change requires manual recompile.\n\n" +
                ...
                alertData: new AlertData(AlertEntryType.UndetectedChange, timestamp, entryType: EntryType.Parent)
            ));
        }

        internal static void CreatePartiallyAppliedEventEntry(PartiallySupportedChange partiallySupportedChange,
            EntryType entryType = EntryType.Standalone, bool detailed = true, DateTime? createdAt = null)
        {
            var timestamp = createdAt ?? DateTime.Now;
            string description;
            if (!partiallySupportedChangeDescriptions.TryGetValue(partiallySupportedChange, out description)) return;
            InsertEntry(new AlertEntry(
                timestamp: timestamp,
                alertType: AlertType.PartiallySupportedChange,
                ...
                entryType: entryType,
                alertData: new AlertData(AlertEntryType.PartiallySupportedChange, timestamp,
                    partiallySupportedChange: partiallySupportedChange, entryType: entryType, detiled: detailed)
            ));
        }
        ...
        internal static void InsertEntry(AlertEntry entry)
        {
            ...
            if (entry.alertType != AlertType.AppliedChange) HotReloadState.ShowingRedDot = true;
        }
        ...
        internal static void HidePartiallySupportedChange(PartiallySupportedChange key)
        {
            ...
            for (var i = EventsTimeline.Count - 1; i >= 0; i--)
            {
                ...
                if (eventEntry.alertData.partiallySupportedChange == key) EventsTimeline.Remove(eventEntry);
            }
        }
