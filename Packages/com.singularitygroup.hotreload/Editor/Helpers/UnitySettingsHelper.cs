...
namespace SingularityGroup.HotReload.Editor
{
    internal class UnitySettingsHelper
    {
        ...
        public static UnitySettingsHelper I = new();
        ...
        private bool EnsureInitialized()
        {
            ...
            try
            {
                ...
                settingsType = settingsType ??
                               (settingsType = typeof(UnityEditor.Editor).Assembly.GetType("UnityEditor.PrefSettings"));
                prefColorType = prefColorType ??
                                (prefColorType = typeof(UnityEditor.Editor).Assembly.GetType("UnityEditor.PrefColor"));
                prefColorProp = prefColorProp ?? (prefColorProp = prefColorType?.GetProperty("Color",
                    BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.Public));
                pref = pref ?? (pref = GetPref(settingsType, prefColorType));
                setMethod = setMethod ?? (setMethod = GetSetMethod(settingsType, prefColorType));
        ...
        {
            var setMethodBase = settingsType?.GetMethod("Set",
        ...
        {
            var prefsMethodBase = settingsType?.GetMethod("Prefs",
                BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Static);
            ...
            if (prefs != null)
                ...
                {
                    var key = kvp.GetType().GetProperty("Key", BindingFlags.Instance | BindingFlags.Public)?.GetMethod
                        .Invoke(kvp, Array.Empty<object>());
                    if (key?.ToString() == currentPlaymodeTintPrefKey)
                        return kvp.GetType().GetProperty("Value", BindingFlags.Instance | BindingFlags.Public)
                            ?.GetMethod.Invoke(kvp, Array.Empty<object>());
                }
        ...
        {
            if (!playmodeTintSupported) return null;
            return (Color)prefColorProp.GetValue(pref);
        }

        public void SetPlaymodeTint(Color color)
        {
            if (!playmodeTintSupported) return;
            prefColorProp.SetValue(pref, color);
            setMethod.Invoke(null, new[] { currentPlaymodeTintPrefKey, pref });
        }
    }
}