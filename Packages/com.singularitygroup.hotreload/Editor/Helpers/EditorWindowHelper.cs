...
namespace SingularityGroup.HotReload.Editor
{
    internal static class EditorWindowHelper
    {
        ...
        private static readonly Dictionary<NotificationStatus, GUIContent> notificationContent = new()
        {
            { NotificationStatus.Patching, new GUIContent("[Hot Reload] Applying patches...") },
            {
                NotificationStatus.NeedsRecompile,
                new GUIContent("[Hot Reload] Unsupported Changes detected! Recompiling...")
            }
        };
        ...
        private static EditorWindow[] sceneWindows =>
            Resources.FindObjectsOfTypeAll(typeof(SceneView)).Cast<EditorWindow>().ToArray();

        private static EditorWindow[] notificationWindows => gameViewWindows.Concat(sceneWindows).ToArray();

        private static NotificationStatus lastNotificationStatus;
        private static DateTime? latestNotificationStartedAt;

        private static bool notificationShownRecently => latestNotificationStartedAt != null &&
                                                         DateTime.UtcNow - latestNotificationStartedAt <
        ...
        internal static void ShowNotification(NotificationStatus notificationType, float maxDuration = 3)
        {
            ...
            var willOverrideNeedsCompileNotification =
