...
namespace SingularityGroup.HotReload.Editor
{
    internal class Spinner
    {
        ...
        private readonly int _rotationPeriod;
        ...
        private Texture2D rotatedTextureLight => _rotatedTextureLight
            ? _rotatedTextureLight
            : _rotatedTextureLight = GetCopy(spinnerTexture);

        private Texture2D rotatedTextureDark =>
            _rotatedTextureDark ? _rotatedTextureDark : _rotatedTextureDark = GetCopy(spinnerTexture);

        internal Texture2D rotatedTexture =>
            HotReloadWindowStyles.IsDarkMode ? rotatedTextureDark : rotatedTextureLight;
        ...
        private Texture2D RotateImage(Texture2D originalTexture, float angle)
        {
            ...
            for (y = 0; y < h; y++)
            {
                ...
                var oldAngle = Mathf.Atan2(dy, dx) * Mathf.Rad2Deg;
                var newAngle = oldAngle + angle;

                var newX = centerX + distance * Mathf.Cos(newAngle * Mathf.Deg2Rad);
                var newY = centerY + distance * Mathf.Sin(newAngle * Mathf.Deg2Rad);
        ...
        public static Texture2D GetCopy(Texture2D tex, TextureFormat format = TextureFormat.RGBA32,
            bool mipChain = false)
        {
            var tmp = RenderTexture.GetTemporary(tex.width, tex.height, 0, RenderTextureFormat.Default,
            ...
            try
            {
                var copy = new Texture2D(tex.width, tex.height, format, mipChain);
