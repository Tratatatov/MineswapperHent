...
namespace SingularityGroup.HotReload.Editor
{
    ...
    {
        private static readonly Dictionary<InvertibleIcon, string> supportedInvertibleIcons = new()
        {
        ...
        private static readonly Dictionary<InvertibleIcon, Texture2D> invertibleIconCache = new();
        private static readonly Dictionary<InvertibleIcon, Texture2D> invertibleIconInvertedCache = new();
        private static readonly Dictionary<string, Texture2D> iconCache = new();

        private static readonly Dictionary<Color, Texture2D> textureColorCache = new();

        private static readonly Dictionary<string, Texture2D> grayTextureCache = new();

        private static readonly Dictionary<string, Color> colorFactor = new()
        {
        ...
        {
            if (!originalTexture) return originalTexture;
            ...
            {
                var originalColor = originalPixels[i];
                var invertedColor = new Color(1 - originalColor.r, 1 - originalColor.g, 1 - originalColor.b,
                    originalColor.a);
                invertedPixels[i] = invertedColor;
            }

            // Create a new texture and set its pixels
            var invertedTexture = new Texture2D(originalTexture.width, originalTexture.height);
        ...
        internal static Texture2D GetInvertibleIcon(InvertibleIcon invertibleIcon)
        {
            ...
            if (!cache.TryGetValue(invertibleIcon, out iconTexture) || !iconTexture)
            {
                ...
                if (HotReloadWindowStyles.IsDarkMode) iconTexture = InvertTextureColor(iconTexture);
        ...
        {
            if (!image1 || !image2) return image1;
            var combinedImage = new Texture2D(Mathf.Max(image1.width, image2.width),
                Mathf.Max(image1.height, image2.height));

            for (var y = 0; y < combinedImage.height; y++)
            for (var x = 0; x < combinedImage.width; x++)
            {
                var color1 = x < image1.width && y < image1.height ? image1.GetPixel(x, y) : Color.clear;
                var color2 = x < image2.width && y < image2.height ? image2.GetPixel(x, y) : Color.clear;
        ...
        internal static Texture2D ConvertToGrayscale(string localIcon)
        {
            ...
            {
                var icon = GetLocalIcon(localIcon);
                // Create a copy of the texture
                var copiedTexture = new Texture2D(icon.width, icon.height, TextureFormat.RGBA32, false);
                ...
                for (var i = 0; i < pixels.Length; i++)
                {
                    var pixel = pixels[i];
                    Color factor;
                    if (!colorFactor.TryGetValue(localIcon, out factor)) factor = new Color(0.299f, 0.587f, 0.114f);
                    var grayscale = factor.r * pixel.r + factor.g * pixel.g + factor.b * pixel.b;
