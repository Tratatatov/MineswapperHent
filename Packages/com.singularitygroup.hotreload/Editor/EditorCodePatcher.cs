...
namespace SingularityGroup.HotReload.Editor
{
    ...
    internal static class EditorCodePatcher
    {
        ...
        private static readonly string[] assetExtensionBlacklist =
        {
            ".cs",
        ...
        public static string[] compileFiles =
        {
            ".asmdef",
            ".asmref",
            ".rsp"
        ...
        static EditorCodePatcher()
        {
            ...
            if (!EditorWindowHelper.IsHumanControllingUs()) return;
            ...
            if (ServerHealthCheck.I.IsServerHealthy) EditorApplication.delayCall += TryPrepareBuildInfo;
            ...
            AssemblyReloadEvents.beforeAssemblyReload += () => { HotReloadTimelineHelper.PersistTimeline(); };
            ...
            DetectEditorStart();
            DetectVersionUpdate();
            HotReload.Demo.Demo.I = new EditorDemo();
...
#pragma warning disable CS0612 // Type or member is obsolete
            if (HotReloadPrefs.RateAppShownLegacy) HotReloadPrefs.RateAppShown = true;
...
#pragma warning restore CS0612 // Type or member is obsolete
            ...
            if (DateTime.Now < new DateTime(2023, 11, 1))
                HotReloadSuggestionsHelper.SetSuggestionsShown(
                    HotReloadSuggestionKind.UnityBestDevelopmentToolAward2023);
            else
                HotReloadSuggestionsHelper.SetSuggestionInactive(HotReloadSuggestionKind
        ...
        internal static UnityLicenseType licenseType { get; }

        internal static bool LoginNotRequired =>
            PackageConst.IsAssetStoreBuild && licenseType != UnityLicenseType.UnityPro;

        internal static bool compileError => _compileError;

        internal static bool Started =>
            ServerHealthCheck.I.IsServerHealthy && DownloadProgress == 1 && StartupProgress?.Item1 == 1;

        internal static bool Starting => (StartedServerRecently() || ServerHealthCheck.I.IsServerHealthy) && !Started &&
                                         starting && patchStatus != PatchStatus.CompileError;

        internal static bool Stopping => stopping && Running;

        internal static bool Compiling => DateTime.UtcNow - startWaitingForCompile < TimeSpan.FromSeconds(5) ||
                                          patchStatus == PatchStatus.Compiling || HotReloadRunTab.recompiling;

        internal static Tuple<float, string> StartupProgress { get; private set; }
        ...
        internal static bool RequestingDownloadAndRun { get; private set; }
        ...
        {
            var eventInfo = typeof(EditorApplication).GetEvent("focusChanged",
                BindingFlags.Static | BindingFlags.NonPublic | BindingFlags.Public);
            var addMethod = eventInfo?.GetAddMethod(true) ?? eventInfo?.GetAddMethod(false);
            if (addMethod != null) addMethod.Invoke(null, new object[] { handler });
        ...
        public static bool TryRecompileUnsupportedChanges()
        {
            ...
            if (!HotReloadPrefs.AutoRecompileUnsupportedChanges
                || (HotReloadTimelineHelper.UnsupportedChangesCount == 0
                    && (!HotReloadPrefs.AutoRecompilePartiallyUnsupportedChanges ||
                        HotReloadTimelineHelper.PartiallySupportedChangesCount == 0))
                || _compileError
            ...
            if (isPlaying) HotReloadState.RecompiledUnsupportedChangesInPlaymode = true;
        ...
        {
            if (string.IsNullOrEmpty(HotReloadPrefs.ActiveDays)) return new HashSet<string>();
        ...
        private static void DetectEditorStart()
        {
            ...
            Task.Run(() =>
            {
                try
                {
                    var lines = File.Exists(sessionFilePath)
                        ? File.ReadAllLines(sessionFilePath)
                        : Array.Empty<string>();

                    long prevSessionId = -1;
                    string prevVersion = null;
                    if (lines.Length >= 2) long.TryParse(lines[1], out prevSessionId);
                    if (lines.Length >= 3) prevVersion = lines[2].Trim();
        ...
        {
            if (serverDownloader.CheckIfDownloaded(HotReloadCli.controller)) return;
            ServerHealthCheck.instance.CheckHealth();
            if (!ServerHealthCheck.I.IsServerHealthy) return;
            ...
            if (restartServer) RestartCodePatcher().Forget();
        }

        private static void UpdateHost()
        {
            RequestHelper.SetServerInfo(new PatchServerInfo(RequestHelper.defaultServerHost, HotReloadState.ServerPort,
        ...
        private static async Task RequestFlushErrorsCore()
        {
            ...
            var waitMs =
                (int)Mathf.Clamp(
                    pollFrequency - (DateTime.Now.Ticks / (float)TimeSpan.TicksPerMillisecond - _lastErrorFlush), 0,
                    pollFrequency);
        ...
        private static async Task FlushErrors()
        {
            ...
            foreach (var responseError in response.errors) Log.Error(responseError);
        }
        ...
        private static void OnIntervalMainThread()
        {
            ...
            {
                if (HotReloadWindow.Current) HotReloadRunTab.RepaintInstant();
                running = ServerHealthCheck.I.IsServerHealthy;
            }

            if (!running) startupCompletedAt = null;
            ...
            if (ServerHealthCheck.I.IsServerHealthy)
            {
                ...
                RequestServerInfo();
                TryPrepareBuildInfo();
                if (!requestingCompile && (!config.patchEditModeOnlyOnEditorFocus || Application.isPlaying ||
                ...
                RequestHelper.PollPatchStatus(resp =>
                {
                    ...
                    if (patchStatus == PatchStatus.Compiling) startWaitingForCompile = null;
                    if (patchStatus == PatchStatus.Patching)
                    {
                        firstPatchAttempted = true;
                        if (HotReloadPrefs.ShowPatchingNotifications)
                            EditorWindowHelper.ShowNotification(EditorWindowHelper.NotificationStatus.Patching, 10);
                    }
                ...
                if (HotReloadPrefs.AllAssetChanges) RequestHelper.PollAssetChanges(HandleAssetChange);
            }

            if (!ServerHealthCheck.I.IsServerHealthy) stopping = false;
            if (StartupProgress?.Item1 == 1) starting = false;
            if (!_requestingFlushErrors && Running) RequestFlushErrors().Forget();
            CheckEditorSettings();
        }
        ...
        private static void HandleAssetChange(string assetPath)
        {
            ...
            foreach (var compileFile in compileFiles)
                ...
                {
                    HotReloadTimelineHelper.CreateErrorEventEntry(
                        $"errors: AssemblyFileEdit: Editing assembly files requires recompiling in Unity. in {assetPath}",
                        EntryType.Foldout);
                    _applyingFailed = true;
                    if (HotReloadPrefs.AutoRecompileUnsupportedChangesImmediately ||
                        InternalEditorUtility.isApplicationActive) TryRecompileUnsupportedChanges();
                    return;
                }
            ...
            foreach (var plugin in plugins)
                ...
                {
                    HotReloadTimelineHelper.CreateErrorEventEntry(
                        $"errors: NativePluginEdit: Editing native plugins requires recompiling in Unity. in {assetPath}",
                        EntryType.Foldout);
                    _applyingFailed = true;
                    if (HotReloadPrefs.AutoRecompileUnsupportedChangesImmediately ||
                        InternalEditorUtility.isApplicationActive) TryRecompileUnsupportedChanges();
        ...
        private static void HandleResponseReceived(MethodPatchResponse response)
        {
            ...
            if (response.patches?.Length > 0)
            {
                ...
                patchResult = CodePatcher.I.RegisterPatches(response, true);
                CodePatcher.I.SaveAppliedPatches(patchesFilePath).Forget();
            }

            var partiallySupportedChangesFiltered =
                new List<PartiallySupportedChange>(response.partiallySupportedChanges ??
                                                   Array.Empty<PartiallySupportedChange>());
            partiallySupportedChangesFiltered.RemoveAll(x =>
                !HotReloadTimelineHelper.GetPartiallySupportedChangePref(x));
            ...
            _appliedUndetected = (patchResult == null || patchResult.patchedMethods.Count == 0) &&
                                 response.removedMethod.Length == 0;

            var allMethods = patchResult?.patchedSMethods.Select(m => GetExtendedMethodName(m));
            if (allMethods == null)
                allMethods = response.removedMethod.Select(m => GetExtendedMethodName(m))
                    .Distinct(StringComparer.OrdinalIgnoreCase);
            else
                allMethods = allMethods.Concat(response.removedMethod.Select(m => GetExtendedMethodName(m)))
                    .Distinct(StringComparer.OrdinalIgnoreCase);
            ...
            else if (_applyingFailed)
            {
                if (partiallySupportedChangesFiltered.Count > 0)
                    foreach (var responsePartiallySupportedChange in partiallySupportedChangesFiltered)
                        HotReloadTimelineHelper.CreatePartiallyAppliedEventEntry(responsePartiallySupportedChange,
                            EntryType.Child);

                foreach (var failure in failuresDeduplicated)
                    HotReloadTimelineHelper.CreateErrorEventEntry(failure, EntryType.Child);
                if (patchResult?.patchFailures.Count > 0)
                    foreach (var failure in patchResult.patchFailures)
                    {
                        var method = failure.Item1;
                        var error = failure.Item2;
                        HotReloadTimelineHelper.CreatePatchFailureEventEntry(error, GetMethodName(method),
                            method.simpleName, EntryType.Child);
                    }

                HotReloadTimelineHelper.CreateReloadFinishedWithWarningsEventEntry(
                    patchedMethodsDisplayNames: allMethods.ToArray());
            ...
            if (compileChecker.hasCompileErrors) HotReloadRunTab.Recompile();
        ...
        private static string GetMethodName(SMethod method)
        {
            ...
            if (spaceIndex > 0) return method.displayName.Substring(spaceIndex);
        ...
        private static void HandleRemovedUnityMethods(SMethod[] removedMethods)
        {
            ...
            foreach (var sMethod in removedMethods)
                ...
                {
                    Log.Warning("Encountered exception in RemoveUnityEventMethod: {0} {1}", ex.GetType().Name,
                        ex.Message);
                }
        }
        ...
        {
            if (HotReloadPrefs.LoggedBurstHint) return;
        ...
        {
            return DateTime.UtcNow - serverStoppedAt < ServerHealthCheck.HeartBeatTimeout ||
                   (!StartedServerRecently() && (StartupProgress?.Item1 ?? 0) == 0);
        }
        ...
        {
            if (requestingStart || StartedServerRecently()) return;
            stopping = false;
            starting = true;
            ...
            try
            {
                ...
                await HotReloadCli.StartAsync(exposeToNetwork, allAssetChanges, disableConsoleWindow, loginData)
        ...
        {
            if (RequestingDownloadAndRun) return false;
            stopping = false;
        ...
        internal static void HandleStatus(LoginStatusResponse resp)
        {
            ...
            var consumptionsChanged = Status?.freeSessionRunning != resp.freeSessionRunning ||
                                      Status?.freeSessionEndTime != resp.freeSessionEndTime;
            var expiresAtChanged = Status?.licenseExpiresAt != resp.licenseExpiresAt;
            if (!LoginNotRequired
            ...
            if (!RequestingLoginInfo && resp.requestError == null) Status = resp;
            ...
            if (startupCompletedAt == null && newStartupProgress.Item1 == 1f) startupCompletedAt = DateTime.UtcNow;
        ...
        {
            if (requestingServerInfo) return;
        ...
        private static async Task RequestServerInfoCore()
        {
            ...
            var waitMs =
                (int)Mathf.Clamp(
                    pollFrequency - (DateTime.Now.Ticks / (float)TimeSpan.TicksPerMillisecond - lastServerPoll), 0,
                    pollFrequency);
            await Task.Delay(waitMs);
