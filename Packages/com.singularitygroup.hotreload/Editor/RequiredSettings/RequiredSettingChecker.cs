...
namespace SingularityGroup.HotReload.Editor
{
    ...
    internal static class AutoRefreshSettingChecker
    {
        ...
        {
            if (HotReloadPrefs.AppliedAutoRefresh) return;
        ...
        public static void Check()
        {
            ...
            if (EditorPrefs.GetInt(autoRefreshKey) != desiredValue) HotReloadPrefs.DefaultAutoRefresh = -1;

#if UNITY_2021_3_OR_NEWER
            if (EditorPrefs.GetInt(autoRefreshModeKey) != desiredValue) HotReloadPrefs.DefaultAutoRefreshMode = -1;
#endif
        }
    ...
    internal static class ScriptCompilationSettingChecker
    {
        ...
        private static readonly int? recompileAfterFinishedPlaying = (int?)typeof(EditorWindow).Assembly
            .GetType("UnityEditor.ScriptChangesDuringPlayOptions")?
            .GetField("RecompileAfterFinishedPlaying", BindingFlags.Static | BindingFlags.Public)?
            .GetValue(null);

        public static void Apply()
        {
            if (HotReloadPrefs.AppliedScriptCompilation) return;
    ...
    {
        private static readonly Color unsupportedPlaymodeColor = new(1f, 0.8f, 0f, 1f);
        private static readonly Color compilePlaymodeErrorColor = new(1f, 0.7f, 0.7f, 1f);

        public static void Apply()
        {
            if (HotReloadPrefs.AppliedEditorTint != null || !UnitySettingsHelper.I.playmodeTintSupported) return;
            var defaultPref = HotReloadPrefs.DefaultEditorTint ?? UnitySettingsHelper.I.GetCurrentPlaymodeColor();
            if (defaultPref == null) return;
        ...
        public static void Check()
        {
            ...
            if (HotReloadPrefs.DefaultEditorTint == null ||
                UnitySettingsHelper.I.GetCurrentPlaymodeColor() != HotReloadPrefs.AppliedEditorTint)
            {
            ...
            if (color != null && color != HotReloadPrefs.AppliedEditorTint) SetPlaymodeTint(color.Value);
        }
    ...
    internal static class CompileMethodDetourer
    {
        ...
        private static readonly List<IDisposable> reverters = new();

        public static void Apply()
        {
            if (detouredMethod) return;
            detouredMethod = true;
            ...
            var originAssetRefreshWithParams = typeof(AssetDatabase).GetMethod(nameof(AssetDatabase.Refresh),
                new[] { typeof(ImportAssetOptions) });
            ...
            var originCompilation =
                typeof(CompilationPipeline).GetMethod(nameof(CompilationPipeline.RequestScriptCompilation),
                    Type.EmptyTypes);
        ...
        private static void DetourMethod(MethodBase original, MethodBase replacement)
        {
            ...
            if (!result.success)
                Debug.LogWarning(
                    $"Detouring {original.Name} method failed. {result.exception?.GetType()} {result.exception}");
            else
